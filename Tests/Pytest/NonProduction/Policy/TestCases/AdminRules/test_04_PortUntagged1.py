...
class PortUntaggedTests(PolicyBase):
    @mark.F_1000043142
    @mark.EXOS
    @mark.P3
    @mark.Admin_Rules

    def test_04_Admin_Rules_Per_Port_Untagged_Packet(self):
        '''[Documentation]  Test_Objective: Verify_policy_rules_associate_traffic_correctly_for_unlearned/learned
                      unicast, broadcast_and_multicast_frames.'''
        
    
        print('  (Step_1) Configure_ports, QoS, and_FDB_for_Policy_traffic_tests.')
        self.localPolicyUdks.Policy_Test_Case_Setup_Admin_Rules_Ports()
        ...
        self.localPolicyUdks.policyUdks.Create_Mac_Source_Admin_Profile_and_Verify_it_was_Created(self.tb.config.netelem1.name,
        ...
        print('  (Verification_1) Verify_100_unlearned_frames_were_sent_and_received, on_vlan_b.')
        ...
        print('  (Verification_2) Verify_100_unlearned_frames_were_sent_and_received, on_vlan_b.')
        self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_Received(
        ...
        print('  (Verification_3) Verify_100_unlearned_frames_were_sent_and_received, on_vlan_c.')
        self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_Received(
                  self.tb.config.tgen_ports.netelem1.port_c,  self.tb.config.tgen_ports.netelem1.port_b,  self.tb.config.src_mac_c, self.tb.config.dst_mac_b,  self.tb.config.vlan_c)
        ...
        self.localPolicyUdks.Transmit_10_Frames_and_Verify_FDB_Entry_is_Learned(self.tb.config.netelem1.name,self.tb.config.tgen_ports.netelem1.port_c,
                  self.tb.config.packet_name_a,  self.tb.config.src_mac_c, self.tb.config.dst_mac_b,   self.tb.config.vlan_c,self.tb.config.netelem1.tgen.port_c.ifname)
        
        time.sleep(2)
        
        print('  (Verification_4) Verify_100_learned_frames_were_sent_and_received, on_vlan_b.')
        self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_Received(
                  self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_b)
    
        print('  (Verification_5) Verify_100_learned_frames_were_sent_and_received, on_vlan_b.')
        self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_Received(
                  self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,  self.tb.config.src_mac_b,  self.tb.config.dst_mac_a,  self.tb.config.vlan_b)
    
        print('  (Verification_6) Verify_100_learned_frames_were_sent_and_received, on_vlan_c.')
        self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_Received(
                  self.tb.config.tgen_ports.netelem1.port_c,  self.tb.config.tgen_ports.netelem1.port_b,  self.tb.config.src_mac_c, self.tb.config.dst_mac_b,  self.tb.config.vlan_c)
    
        print('  (Verification_7) Verify_100_broadcast_frames_were_sent_and_received, on_vlan_b.')
        self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_Received(
                  self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,  self.tb.config.src_mac_a,  self.tb.config.bcast_mac,  self.tb.config.vlan_b)
    
        print('  (Verification_8) Verify_100_broadcast_frames_were_sent_and_received, on_vlan_b.')
        self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_Received(
                  self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,  self.tb.config.src_mac_b,  self.tb.config.bcast_mac,  self.tb.config.vlan_b)
    
        print('  (Verification_9) Verify_100_broadcast_frames_were_sent_and_received, on_vlan_c.')
        self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_Received(
                  self.tb.config.tgen_ports.netelem1.port_c,  self.tb.config.tgen_ports.netelem1.port_b,  self.tb.config.src_mac_c,  self.tb.config.bcast_mac,  self.tb.config.vlan_c)
    
        print('  (Verification_7) Verify_100_multicast_frames_were_sent_and_received, on_vlan_b.')
        self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_Received(
                  self.tb.config.tgen_ports.netelem1.port_a, self.tb.config.tgen_ports.netelem1.port_b, self.tb.config.src_mac_a, self.tb.config.mcast_mac, self.tb.config.vlan_b)
    
        print('  (Verification_7) Verify_100_multicast_frames_were_sent_and_received, on_vlan_b.')
        self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_Received(
                  self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,  self.tb.config.src_mac_b,  self.tb.config.mcast_mac,  self.tb.config.vlan_b)
    
        print('  (Verification_8) Verify_100_multicast_frames_were_sent_and_received, on_vlan_c.')
        self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_Received(
                  self.tb.config.tgen_ports.netelem1.port_c,  self.tb.config.tgen_ports.netelem1.port_b,  self.tb.config.src_mac_c,  self.tb.config.mcast_mac,  self.tb.config.vlan_c)
    ...
    def Test_Case_Cleanup(self):
        ...
        self.localPolicyUdks.fdbUdks.Remove_Static_FDB_Entry_and_Validate_It_Does_Not_Exist(
                                    self.tb.config.netelem1.name, self.tb.config.dst_mac_b, self.tb.config.vlan_c, self.tb.config.netelem1.tgen.port_b.ifname)
        ...
        self.localPolicyUdks.vlanUdks.Remove_VLAN_and_Verify_it_is_Removed(self.tb.config.netelem1.name,  self.tb.config.vlan_c)
