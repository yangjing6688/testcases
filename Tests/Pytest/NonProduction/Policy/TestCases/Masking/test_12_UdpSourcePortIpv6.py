...
class UdpSourcePortIpv6Tests(PolicyBase):
    @mark.F_129411
    @mark.EXOS
    @mark.P3
    @mark.Policy_Masking
    @mark.NIGHTLY
    def testing_12_Masking_Precedence_IPv6_UDPSourcePortIP(self):
        ...
        self.localPolicyUdks.policy_set_rule_udpsourceportip(         self.tb.config.netelem1.name,  self.tb.config.profile_a,  self.tb.config.mask_l4ip_b,  mask='8',
                                                        drop=True)
        ...
        self.localPolicyUdks.Transmit_100_Untagged_IPv6_UDP_Frames_and_Verify_NOT_Received(
                  ...
                  self.tb.config.src_ip_a,       self.tb.config.dst_ip_a,  src_port=self.tb.config.l4_port_a,          dst_port=self.tb.config.l4_port_c)
    
        print('  (Verification_2) Verify_100_unlearned_frames_were_sent_and_NOT_received.')
        self.localPolicyUdks.Transmit_100_Tagged_IPv6_UDP_Frames_and_Verify_NOT_Received(
                  self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,  self.tb.config.src_mac_a,  self.tb.config.dst_mac_a,  self.tb.config.vlan_a,
                  self.tb.config.src_ip_a,       self.tb.config.dst_ip_a,  src_port=self.tb.config.l4_port_a,          dst_port=self.tb.config.l4_port_c)
    
        print('  (Step_3) Create_additional_Policy_Rules.')
        self.localPolicyUdks.policy_set_rule_udpsourceportip(        self.tb.config.netelem1.name,  self.tb.config.profile_a,self.tb.config.mask_l4ip_b,  mask='16',
                                                       forward=True)
        ...
        self.localPolicyUdks.Transmit_100_Untagged_IPv6_UDP_Frames_and_Verify_Received(
                  self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,  self.tb.config.src_mac_a,  self.tb.config.dst_mac_a,  self.tb.config.vlan_a,
                  self.tb.config.src_ip_a,       self.tb.config.dst_ip_a,  src_port=self.tb.config.l4_port_a,          dst_port=self.tb.config.l4_port_c)
    
        print('  (Verification_4) Verify_100_unlearned_frames_were_sent_and_received.')
        self.localPolicyUdks.Transmit_100_Tagged_IPv6_UDP_Frames_and_Verify_Received(
        self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,  self.tb.config.src_mac_a,  self.tb.config.dst_mac_a,  self.tb.config.vlan_a,
                  self.tb.config.src_ip_a,       self.tb.config.dst_ip_a,  src_port=self.tb.config.l4_port_a,          dst_port=self.tb.config.l4_port_c)
