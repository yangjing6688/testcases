...
class EtherTypeTests(PolicyBase):    
    @mark.F_1000043231
    @mark.EXOS
    @mark.P3
    @mark.Policy_Masking
    ...
    def test_20_Masking_Precedence_EtherType(self):
        '''[Documentation]  Test_Objective: Verify_ethertype_policy_rules_drop_or_forward_traffic_correctly_when_using_masks.'''
        ...
        self.localPolicyUdks.policy_set_rule_ether(self.tb.config.netelem1.name,  self.tb.config.profile_a, self.tb.config.type_ipv6,  mask='4',
                                                        drop=True)
        self.localPolicyUdks.policy_verify_rule_ethertype_exists(         self.tb.config.netelem1.name,  self.tb.config.profile_a,    self.tb.config.mask_type_a,  mask='4')
        ...
        self.localPolicyUdks.Transmit_100_Untagged_IPv6_Frames_and_Verify_NOT_Received(
                  ...
                  self.tb.config.src_ip6_a,     self.tb.config.dst_ip6_a)
        ...
        self.localPolicyUdks.policy_set_rule_ether(self.tb.config.netelem1.name,  self.tb.config.profile_a,  self.tb.config.type_ipv6,    mask='8', forward=True)
        self.localPolicyUdks.policy_verify_rule_ethertype_exists(  self.tb.config.netelem1.name,  self.tb.config.profile_a,  self.tb.config.mask_type_b,  mask='8')
        time.sleep(int(self.tb.config.policy_delay))  # Delay_for_EXOS_policy_hardware_config_batching.
        ...
        self.localPolicyUdks.policy_set_rule_ether(self.tb.config.netelem1.name,  self.tb.config.profile_a,  self.tb.config.type_ipv6,  mask='12', drop=True)
        self.localPolicyUdks.policy_verify_rule_ethertype_exists(  self.tb.config.netelem1.name,  self.tb.config.profile_a,  self.tb.config.mask_type_c,  mask='12')
        time.sleep(int(self.tb.config.policy_delay))  # Delay_for_EXOS_policy_hardware_config_batching.
        ...
        self.localPolicyUdks.policy_set_rule_ether(self.tb.config.netelem1.name,  self.tb.config.profile_a, self.tb.config.type_ipv6,  mask='16', forward=True)
        self.localPolicyUdks.policy_verify_rule_ethertype_exists(  self.tb.config.netelem1.name,  self.tb.config.profile_a,  self.tb.config.mask_type_d,  mask='16')
        time.sleep(int(self.tb.config.policy_delay))  # Delay_for_EXOS_policy_hardware_config_batching.
