...
class TcpDestPortIpv6Tests(PolicyBase):
    @mark.F_129414
    @mark.EXOS
    @mark.P3
    @mark.Policy_Masking
    @mark.NIGHTLY
    def test_11_Masking_Precedence_IPv6_TCPDestPortIP(self):
        ...
        self.localPolicyUdks.policy_set_rule_tcpdestportip(           self.tb.config.netelem1.name,  self.tb.config.profile_a,    self.tb.config.mask_l4ip_b,  mask='8',
                                                        drop=True)
        ...
        self.localPolicyUdks.Transmit_100_Untagged_IPv6_TCP_Frames_and_Verify_NOT_Received(
                  ...
                  self.tb.config.src_ip6_a,     self.tb.config.dst_ip6_a,  src_port=self.tb.config.l4_port_b,          dst_port=self.tb.config.l4_port_a)
        ...
        self.localPolicyUdks.policy_set_rule_tcpdestportip(        self.tb.config.netelem1.name,  self.tb.config.profile_a,  self.tb.config.mask_l4ip_b,  mask='16',
                                                     forward=True)
