...
class CosPvidStatusDisabledTests(PolicyBase):
     @mark.F_1000043375
     @mark.EXOS
     @mark.P1
     @mark.Policy_Profile
     def test_01_Policy_Profile_CoS_and_PVID_Status_Disabled(self):
         '''[Documentation]  Test_Objective: Verify_a_policy_profile_having_cos_and_pvid_status_disabled_untagged, priority tagged_and_vlan_tagged_packets_are_handled_correctly.'''
         ...
         self.localPolicyUdks.Create_Policy_Profile_with_PVID_Status_and_CoS_Status_Disabled(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.vlan_b,self.tb.config.cos_5)
