...
class CosPvidStatusEnabledTests(PolicyBase):
     @mark.F_1000043384
     @mark.EXOS
     @mark.P2
     @mark.Policy_Profile
     def test_08_Policy_Profile_CoS_and_PVID_Status_Enabled(self):
         '''[Documentation]  Test_Objective: Verify_a_policy_profile_having_cos-status & pvid-status_enabled_untagged, priority tagged_and_vlan_tagged_packets_are_handled_correctly.'''
         ...
         self.localPolicyUdks.Create_Policy_Profile_with_PVID(PVID_Status,COS,and_COS_Status_Enabled,self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.vlan_b,self.tb.config.cos_5)
         ...
         self.localPolicyUdks.Transmit_100_Priority_Tagged_Frames_and_Verify_Priority_has_NOT_Changed(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_b,self.tb.config.dst_mac_a,self.tb.config.vlan_b,self.tb.config.priority_6)
