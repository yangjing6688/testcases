...
class StaticVsPolicyUntaggedPrecedenceTests(PolicyBase):
     @mark.F_1000043391
     @mark.EXOS
     @mark.P3
     @mark.Policy_Profile
     def test_16_Policy_Profile_Static_vs_Policy_Untagged_VLAN_Precedence(self):
         '''[Documentation]  Test_Objective: Verify_static_vlan_egress_settings_have_precedence_over_a_policy_profile configured_with_untagged-vlans.'''
         ...
         self.localPolicyUdks.vlanUdks.Add_Ports_to_Tagged_Egress_for_VLAN_and_Verify_it_is_Added(self.tb.config.netelem1.name,self.tb.config.vlan_b,self.tb.config.netelem1.tgen.port_a.ifname)
         ...
         self.localPolicyUdks.Configure_Policy_Profile_Untagged_VLANs(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.vlan_b)
         self.localPolicyUdks.Policy_Profile_Untagged_VLAN_Should_Contain(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.vlan_b)
         ...
         print('  (Verification_1) Verify_100_untagged_frames_were_sent_and_received.')
         self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_Received_Untagged(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a)
         print('  (Verification_2) Verify_100_untagged_frames_were_sent_and_received, with_a_VID_of_vlan_b.')
         self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_Received(self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.dst_mac_a,self.tb.config.src_mac_a,self.tb.config.vlan_b)
