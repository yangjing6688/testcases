...
class CosStatusEnabledTests(PolicyBase):
     @mark.F_1000043381
     @mark.EXOS
     @mark.P3
     @mark.Policy_Profile
     def test_05_Policy_Profile_CoS_Status_Enabled(self):
         '''[Documentation]  Test_Objective: Verify_a_policy_profile_having_cos-status_enabled_untagged, priority_tagged_and vlan_tagged_packets_are_handled_correctly.'''
         ...
         self.localPolicyUdks.Create_Policy_Profile_with_COS_and_COS_Status_Enabled(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.cos_5)
