...
class MultiMatchForwardingTests(PolicyBase): 
    @mark.F_1000_0206
    @mark.EXOS
    @mark.P3
    @mark.Precedence_ACL
    ...
    def test_06_Access_List_Multiple_Match_Mixed_Mask_Forwarding_Test(self):
        #[Documentation]  Test_Objective: Verify_Access-List_with_more_than_one_match_condition_Forwards_properly. (policy_profile_defaulted_to_drop)
        ...
        self.localPolicyUdks.policyUdks.Create_Policy_Profile_With_PVID_and_PVID_Status_Enabled(    
                                                         self.tb.config.netelem1.name,  self.tb.config.profile_b,  '0')
    ...
    #
        ...
        self.localPolicyUdks.networkElementPolicyGenKeywords.policy_set_access_list(self.tb.config.netelem1.name,'IpV4Sock','ipV4SockFrwd',
            'ipdestsocket ' + self.tb.config.dst_ip_a+':'+self.tb.config.l4_port_a + ' mask 48 ' + ' ipsourcesocket ' + self.tb.config.src_ip_b +':'+  self.tb.config.l4_port_b + ' mask 48', 
            'forward')
        self.localPolicyUdks.networkElementPolicyGenKeywords.policy_verify_acl_ipdestsocket(self.tb.config.netelem1.name,'IpV4Sock', 'ipV4SockFrwd',
                                                                                             self.tb.config.dst_ip_a + ':' + self.tb.config.l4_port_a,   '48')
        self.localPolicyUdks.networkElementPolicyGenKeywords.policy_verify_acl_ipsourcesocket(self.tb.config.netelem1.name,'IpV4Sock','ipV4SockFrwd', self.tb.config.src_ip_b + ':' +self.tb.config.l4_port_b,'48')
        self.localPolicyUdks.networkElementPolicyGenKeywords.policy_verify_acl_action_forward(self.tb.config.netelem1.name,'IpV4Sock','ipV4SockFrwd')
    ...
    #
        print('  (Step_3) Validate_TCP_and_UDP_Traffic_with_configured_Access-List_to_Forward_IpV4_specified_TCP_and_UDP_Frames')
    #
    #      Source_and_dest_IP/Port_match_TCP_frame_should_be_forwarded.
    #
        print('  (Verification_3a) Verify_100_TCP_frames_were_sent_and_forwarded.')
    ...
    #
    #      Source_Dest_port "C" does_not_match_should_drop_by_default.
    #
        print('  (Verification_3b) Verify_100_TCP_frames_were_sent_and_dropped.')
    ...
    #
    #      Source_Port "C" does_not_match_should_drop_by_default.    
    # level to defaults
        confidenceLevel = 10
    ...
    #
    #      Dest_IP "C" does_not_match_should_drop_by_default.
    #
        if  confidenceLevel >= highConfidenceLevel:
            self.localPolicyUdks.Transmit_100_Untagged_TCP_Frames_and_Verify_NOT_Received(
            self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_b,
    ...
    #
    #      Source_IP "C" does_not_match_should_drop_by_default.
    #
        if  confidenceLevel >= highConfidenceLevel:
            self.localPolicyUdks.Transmit_100_Untagged_TCP_Frames_and_Verify_NOT_Received
            self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_b,
    ...
    #
    #      Source_and_dest_IP/Port_match_UDP_frame_should_be_forwarded.
    #
        if  confidenceLevel >= highConfidenceLevel:
    ...
    #      Source_Dest_port "C" does_not_match_should_drop_by_default.
    #
            print('  (Verification_3d) Verify_100_UDP_frames_were_sent_and_dropped.')
    ...
    #
            ...
            self.localPolicyUdks.networkElementPolicyGenKeywords.policy_set_access_list(self.tb.config.netelem1.name,   'IpV4Sock', 'tcpV4SockFrwd',
            'tcpdestportip ' +  self.tb.config.l4_port_a + ':' + self.tb.config.dst_ip_a + ' mask 48 '+ 'tcpsourceportip ' + self.tb.config.l4_port_b + ':' + self.tb.config.src_ip_b
            +  ' mask 48', 'forward')
            self.localPolicyUdks.networkElementPolicyGenKeywords.policy_verify_acl_tcpdestportip(self.tb.config.netelem1.name, 'IpV4Sock','tcpV4SockFrwd',  self.tb.config.l4_port_a + ':' + self.tb.config.dst_ip_a,
                                                                                                 '48')
            self.localPolicyUdks.networkElementPolicyGenKeywords.policy_verify_acl_tcpsourceportip(self.tb.config.netelem1.name,'IpV4Sock','tcpV4SockFrwd',   self.tb.config.l4_port_b + ':' + self.tb.config.src_ip_b,'48')
            self.localPolicyUdks.networkElementPolicyGenKeywords.policy_verify_acl_action_forward(self.tb.config.netelem1.name,'IpV4Sock','tcpV4SockFrwd')
    
            print('   (Step_5) Remove_prior_rule_IpV4SockFrwd_from_access_list')
            self.localPolicyUdks.networkElementPolicyGenKeywords.policy_clear_access_list_rule(self.tb.config.netelem1.name,'IpV4Sock','ipV4SockFrwd')
            time.sleep(int(self.tb.config.policy_delay))  # Delay_for_EXOS_policy_hardware_config_batching.
    #
    #      Should_hit_drop_if_src_ip "B" and_port "B" and_dst_ip "A" and_port "A" match.
    #
    ...
    #      Source_and_dest_IP/Port_match_TCP_frame_should_be_forwarded.
    #
            print('  (Step_6) Validate_TCP_and_UDP_Traffic_with_configured_Access-List_to_forward_IpV4_specified_TCP_Frames')
    #
            print('  (Verification_6a) Verify_100_TCP_frames_were_sent_and_forwarded.')
            self.localPolicyUdks.Transmit_100_Untagged_TCP_Frames_and_Verify_Received(
            self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,    self.tb.config.src_mac_a,    self.tb.config.dst_mac_a,    self.tb.config.vlan_b,
            self.tb.config.src_ip_b,    self.tb.config.dst_ip_a,    self.tb.config.l4_port_b,    self.tb.config.l4_port_a)
    #
    #      Source_Dest_port "C" does_not_match_should_drop_by_default.
    #
            print('  (Verification_6b) Verify_100_TCP_frames_were_sent_and_dropped.')
            self.localPolicyUdks.Transmit_100_Untagged_TCP_Frames_and_Verify_NOT_Received(
            self.tb.config.tgen_ports.netelem1.port_a,    self.tb.config.tgen_ports.netelem1.port_b,    self.tb.config.src_mac_a,    self.tb.config.dst_mac_a,    self.tb.config.vlan_b,
            self.tb.config.src_ip_b,    self.tb.config.dst_ip_a,    self.tb.config.l4_port_b,    self.tb.config.l4_port_c)
    #
    ...
    #
        if  confidenceLevel >= highConfidenceLevel:
            print('  (Verification_6c) Verify_100_UDP_frames_were_sent_and_dropped (match_is_for_TCP_only).')
            self.localPolicyUdks.Transmit_100_Untagged_UDP_Frames_and_Verify_NOT_Received(
            self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,    self.tb.config.src_mac_a,    self.tb.config.dst_mac_a,    self.tb.config.vlan_b,
            self.tb.config.src_ip_b, self.tb.config.dst_ip_a,    self.tb.config.l4_port_b,    self.tb.config.l4_port_a)
    #
    ...
    #
        ...
        self.localPolicyUdks.networkElementPolicyGenKeywords.policy_set_access_list(self.tb.config.netelem1.name, 'IpV4Sock','udpV4SockFrwd',
        'udpdestportip ' + self.tb.config.l4_port_a +':' +self.tb.config.dst_ip_a + ' mask 48 '+' udpsourceportip ' + self.tb.config.l4_port_b + ':' + self.tb.config.src_ip_b + ' mask 48','forward')
        self.localPolicyUdks.networkElementPolicyGenKeywords.policy_verify_acl_udpdestportip(self.tb.config.netelem1.name,'IpV4Sock','udpV4SockFrwd',self.tb.config.l4_port_a+':'+self.tb.config.dst_ip_a,'48')
        self.localPolicyUdks.networkElementPolicyGenKeywords.policy_verify_acl_udpsourceportip(self.tb.config.netelem1.name,'IpV4Sock','udpV4SockFrwd',self.tb.config.l4_port_b+':'+self.tb.config.src_ip_b,'48')
        self.localPolicyUdks.networkElementPolicyGenKeywords.policy_verify_acl_action_forward(self.tb.config.netelem1.name,'IpV4Sock','udpV4SockFrwd')
        self.localPolicyUdks.networkElementPolicyGenKeywords.policy_clear_access_list_rule(self.tb.config.netelem1.name,'IpV4Sock','tcpV4SockFrwd')
        time.sleep(int(self.tb.config.policy_delay))  # Delay_for_EXOS_policy_hardware_config_batching.
    #
    #      Should_hit_drop_if_src_ip "B" and_port "B" and_dst_ip "A" and_port "A" match.
    #
    ...
    #      Source_and_dest_IP/Port_match_UDP_frame_should_be_forwarded.
    #
        print('  (Step_8) Validate_TCP_and_UDP_Traffic_with_configured_Access-List_to_Forwarded_IpV4_specified_UDP_Frames')
    #
        print('  (Verification_8a) Verify_100_TCP_frames_were_sent_and_dropped (match_is_for_UDP_only).')
        self.localPolicyUdks.Transmit_100_Untagged_TCP_Frames_and_Verify_NOT_Received(
            self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,    self.tb.config.src_mac_a,    self.tb.config.dst_mac_a,    self.tb.config.vlan_b,
            self.tb.config.src_ip_b,    self.tb.config.dst_ip_a,    self.tb.config.l4_port_b,    self.tb.config.l4_port_a)
    #
    ...
    #      Source_and_dest_IP/Port_match_UDP_frame_should_be_forwarded.
    #
            print('  (Verification_8b) Verify_100_UDP_frames_were_sent_and_forwarded.')
            self.localPolicyUdks.Transmit_100_Untagged_UDP_Frames_and_Verify_Received(
                self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,    self.tb.config.src_mac_a,    self.tb.config.dst_mac_a,    self.tb.config.vlan_b,
                self.tb.config.src_ip_b,    self.tb.config.dst_ip_a,    self.tb.config.l4_port_b,    self.tb.config.l4_port_a)
    #
    ...
    #      Source_IP "C" does_not_match_should_drop_by_default.
        ...
        if  confidenceLevel >= highConfidenceLevel:
            self.localPolicyUdks.Transmit_100_Untagged_UDP_Frames_and_Verify_NOT_Received(
             self.tb.config.tgen_ports.netelem1.port_a,    self.tb.config.tgen_ports.netelem1.port_b,    self.tb.config.src_mac_a,    self.tb.config.dst_mac_a,    self.tb.config.vlan_b,
              self.tb.config.src_ip_b,    self.tb.config.dst_ip_a,    self.tb.config.l4_port_a,    self.tb.config.l4_port_a)
    #
    ...
    def Test_Case_Cleanup(self):
        ...
        self.localPolicyUdks.fdbUdks.Remove_Static_FDB_Entry_and_Validate_It_Does_Not_Exist(self.tb.config.netelem1.name, self.tb.config.dst_mac_a,self.tb.config.vlan_a,
                                                                                            self.tb.config.netelem1.tgen.port_b.ifname)
