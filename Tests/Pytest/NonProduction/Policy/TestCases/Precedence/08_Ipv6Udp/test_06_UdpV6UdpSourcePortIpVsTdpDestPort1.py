...
class UdpSourcePortIpVsTdpDestPortTests(PolicyBase):
     @mark.F_1000043308
     @mark.EXOS
     @mark.P3
     @mark.Precedence
     ...
     @mark.NIGHTLY
     def test_06_IPv6_UDP_Precedence_UDP_Source_Port_vs_UDP_Dest_Port(self):
         ...
         self.localPolicyUdks.policy_set_rule_udpdestportip(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_port_c,forward=True,cos=self.tb.config.cos_5)
         ...
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
         self.localPolicyUdks.Transmit_100_Untagged_IPv6_UDP_Frames_and_Verify_Priority_Changed(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.priority_4,self.tb.config.src_ip6_a,self.tb.config.dst_ip6_a,src_port=self.tb.config.l4_port_a,dst_port=self.tb.config.l4_port_c)
