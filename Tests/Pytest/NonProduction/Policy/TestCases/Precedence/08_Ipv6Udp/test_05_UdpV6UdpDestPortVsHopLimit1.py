...
class UdpDestPortVsHopLimitTests(PolicyBase):
     @mark.F_1000043307
     @mark.EXOS
     @mark.P3
     @mark.Precedence
     @mark.Precedence_IpV6Udp
     @mark.NIGHTLY
     def test_05_IPv6_UDP_Precedence_UDP_Dest_Port_vs_Hop_Limit(self):
         ...
         self.localPolicyUdks.policy_set_rule_ipttl(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.ttl_a,forward=True,cos=self.tb.config.cos_5)
         ...
         self.localPolicyUdks.policy_verify_rule_udpdestportip_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_port_c)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
         self.localPolicyUdks.Transmit_100_Untagged_IPv6_UDP_Frames_and_Verify_NOT_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.src_ip6_a,self.tb.config.dst_ip6_a,hop_limit=self.tb.config.ttl_a,traffic_class=self.tb.config.tos_a,src_port=self.tb.config.l4_port_a,dst_port=self.tb.config.l4_port_c)
         print('  (Step_4) Create_policy_profile, rules, and_admin_port_rule.')
         self.localPolicyUdks.policy_set_rule_udpdestportip(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_port_c,forward=True,cos=self.tb.config.cos_4)
         self.localPolicyUdks.policy_verify_rule_udpdestportip_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_port_c,cos=self.tb.config.cos_4)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
