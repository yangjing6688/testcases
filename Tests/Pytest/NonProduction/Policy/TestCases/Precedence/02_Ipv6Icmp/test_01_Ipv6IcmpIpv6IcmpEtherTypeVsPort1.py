...
class EtherTypeVsPortTests(PolicyBase):
     @mark.F_1000043242
     @mark.EXOS
     @mark.P3
     @mark.Precedence
     ...
     @mark.NIGHTLY
     def test_01_IPv6_ICMP_Precedence_EtherType_vs_Port(self):
         '''[Documentation]  Test_Objective: Verify_Ethertype_rule_takes_precedence_over_Port_rule.'''
         ...
         self.localPolicyUdks.policy_set_rule_port(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.netelem1.tgen.port_a.ifname,forward=True,cos=self.tb.config.cos_5)
         ...
         self.localPolicyUdks.policy_set_rule_ether(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.type_ipv6,drop=True)
         self.localPolicyUdks.policy_verify_rule_ethertype_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.type_ipv6)
         time.sleep(int(self.tb.config.policy_delay))
         ...
         self.localPolicyUdks.policy_set_rule_ether(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.type_ipv6,forward=True,cos=self.tb.config.cos_4)
