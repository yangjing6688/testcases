...
class MacDestVsIpv6DestTests(PolicyBase):
     @mark.F_1000043248
     @mark.EXOS
     @mark.P2
     @mark.Precedence
     ...
     @mark.NIGHTLY
     def test_07_IPv6_ICMP_Precedence_MAC_Dest_vs_IPv6_Dest(self):
         ...
         self.localPolicyUdks.policy_set_rule_ip6dest(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_ip6_a,forward=True,cos=self.tb.config.cos_5)
         self.localPolicyUdks.policy_verify_rule_ip6dest_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_ip6_a,forward=True,cos=self.tb.config.cos_5)
         ...
         self.localPolicyUdks.policy_set_rule_macdest(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_mac_a,drop=True)
         self.localPolicyUdks.policy_verify_rule_macdest_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_mac_a,drop=True)
         time.sleep(int(self.tb.config.policy_delay))
         ...
         self.localPolicyUdks.policy_set_rule_macdest(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_mac_a,forward=True,cos=self.tb.config.cos_4)
         self.localPolicyUdks.policy_verify_rule_macdest_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_mac_a,forward=True,cos=self.tb.config.cos_4)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
