...
class NextHeaderVsEtherTypeTests(PolicyBase):
     @mark.F_1000043243
     @mark.EXOS
     @mark.P1
     @mark.Precedence
     @mark.Precedence_IpV6Icmp
     @mark.NIGHTLY
     def test_02_IPv6_ICMP_Precedence_NextHeader_vs_EtherType(self):
         '''[Documentation]  Test_Objective: Verify_ipprotocol_rule_takes_precedence_over_ethertype_rule.'''
         ...
         self.localPolicyUdks.networkElementPolicyGenKeywords.policy_set_rule_ether(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.type_ipv6,forward=True,cos=self.tb.config.cos_5)
         self.localPolicyUdks.policy_verify_rule_ethertype_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.type_ipv6,forward=True,cos=self.tb.config.cos_5)
         ...
         self.localPolicyUdks.Transmit_100_Untagged_IPv6_ICMP_Frames_and_Verify_Priority_Changed(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.priority_5,self.tb.config.src_ip6_a,self.tb.config.dst_ip6_a)
         print('  (Step_3) Create_policy_profile, rules, and_admin_port_rule.')
         self.localPolicyUdks.networkElementPolicyGenKeywords.policy_set_rule_ipproto(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.proto_icmp6,drop=True)
         self.localPolicyUdks.networkElementPolicyGenKeywords.policy_verify_rule_ether_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.proto_icmp6,drop=True)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
         self.localPolicyUdks.Transmit_100_Untagged_IPv6_ICMP_Frames_and_Verify_NOT_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.src_ip6_a,self.tb.config.dst_ip6_a)
         print('  (Step_4) Create_policy_profile, rules, and_admin_port_rule.')
         self.localPolicyUdks.networkElementPolicyGenKeywords.policy_set_rule_ipproto(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.proto_icmp6,forward=True,cos=self.tb.config.cos_4)
         self.localPolicyUdks.networkElementPolicyGenKeywords.policy_verify_rule_ether_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.proto_icmp6,forward=True,cos=self.tb.config.cos_4)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
