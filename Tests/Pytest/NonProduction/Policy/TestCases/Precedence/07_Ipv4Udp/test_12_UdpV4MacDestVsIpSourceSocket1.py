...
class MacDestVsIpSourceSocketTests(PolicyBase):
     @mark.F_1000043301
     @mark.EXOS
     @mark.P2
     @mark.Precedence
     ...
     @mark.NIGHTLY
     def test_12_IPv4_UDP_Precedence_MAC_Dest_vs_IP_Source(self):
         ...
         self.localPolicyUdks.policy_set_rule_ipsourcesocket(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.src_socket,forward=True,cos=self.tb.config.cos_5)
         self.localPolicyUdks.policy_verify_rule_ipsourcesocket_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.src_socket,cos=self.tb.config.cos_5)
         ...
         self.localPolicyUdks.policy_verify_rule_macdest_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_mac_a)
         time.sleep(int(self.tb.config.policy_delay))
         ...
         self.localPolicyUdks.policy_verify_rule_macdest_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_mac_a,cos=self.tb.config.cos_4)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
