...
class IpTosVsIpProtocolTests(PolicyBase):
     ...
     @mark.NIGHTLY
     def test_03_IPv4_TCP_Precedence_IP_ToS_vs_IP_Protocol(self):
         ...
         self.localPolicyUdks.policy_set_rule_ipproto(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.proto_tcp,forward=True,cos=self.tb.config.cos_5)
         self.localPolicyUdks.policy_verify_rule_ipproto_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.proto_tcp,cos=self.tb.config.cos_5)
