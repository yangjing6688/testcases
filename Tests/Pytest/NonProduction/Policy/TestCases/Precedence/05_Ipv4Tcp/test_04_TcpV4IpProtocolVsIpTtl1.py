...
class IpProtocolVsIpTtlTests(PolicyBase):
     @mark.F_1000043273
     @mark.EXOS
     @mark.P3
     def test_04_IPv4_TCP_Precedence_IP_Protocol_vs_IP_TTL(self):
         ...
         self.localPolicyUdks.policy_verify_rule_ipproto_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.proto_tcp)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
         self.localPolicyUdks.Transmit_100_Untagged_TCP_Frames_and_Verify_NOT_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.src_ip_a,self.tb.config.dst_ip_a,ttl=self.tb.config.ttl_a,src_port=self.tb.config.l4_port_a,dst_port=self.tb.config.l4_port_c)
         ...
         self.localPolicyUdks.policy_verify_rule_ipproto_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.proto_tcp,cos=self.tb.config.cos_4)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
         self.localPolicyUdks.Transmit_100_Untagged_TCP_Frames_and_Verify_Priority_Changed(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.priority_4,self.tb.config.src_ip_a,self.tb.config.dst_ip_a,ttl=self.tb.config.ttl_a,src_port=self.tb.config.l4_port_a,dst_port=self.tb.config.l4_port_c)
