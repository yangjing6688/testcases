...
class TcpSourcePortIpVsTcpDestPortTests(PolicyBase):
     @mark.F_1000043276
     @mark.EXOS
     @mark.P2
     @mark.Precedence
     @mark.Precedence_IpV4Tcp
     @mark.NIGHTLY
     def test_07_IPv4_TCP_Precedence_TCP_Source_Port_IP_vs_TCP_Dest_Port_IP(self):
         ...
         self.localPolicyUdks.policy_set_rule_tcpdestportip(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_dst_socket,forward=True,cos=self.tb.config.cos_5)
         self.localPolicyUdks.policy_verify_rule_tcpdestportip_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_dst_socket,cos=self.tb.config.cos_5)
         ...
         self.localPolicyUdks.policy_verify_rule_tcpsourceportip_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_port_a)
         time.sleep(int(self.tb.config.policy_delay))
         ...
         self.localPolicyUdks.policy_set_rule_tcpsourceportip(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_port_a,forward=True,cos=self.tb.config.cos_4)
         self.localPolicyUdks.policy_verify_rule_tcpsourceportip_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_port_a,cos=self.tb.config.cos_4)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
