...
class IpTtlVsIpTosTests(PolicyBase):
     ...
     @mark.P1
     @mark.Precedence
     @mark.Precedence_IpV4Icmp
     @mark.NIGHTLY
     def test_04_IPv4_ICMP_Precedence_IP_TTL_vs_IP_ToS(self):
         ...
         self.localPolicyUdks.Transmit_100_Untagged_IPv4_ICMP_Frames_and_Verify_NOT_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.src_ip_a,self.tb.config.dst_ip_a,ttl=self.tb.config.ttl_a,tos=self.tb.config.tos_a)
