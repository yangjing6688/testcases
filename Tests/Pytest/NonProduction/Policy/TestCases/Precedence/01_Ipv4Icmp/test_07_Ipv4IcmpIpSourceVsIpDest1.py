...
class IpSourceVsIpDestTests(PolicyBase):
     @mark.F_126888
     @mark.EXOS
     @mark.P3
     @mark.Precedence
     ...
     @mark.NIGHTLY
     def test_07_IPv4_ICMP_Precedence_IP_Source_vs_IP_Destination(self):
         '''[Documentation]  Test_Objective: Verify_ipsourcesocket_rule_takes_precedence_over_ipdestsocket_rule.'''
         ...
         self.localPolicyUdks.policy_set_rule_ipdestsocket(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_ip_a,forward=True,cos=self.tb.config.cos_5)
         self.localPolicyUdks.policy_verify_rule_ipdestsocket_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_ip_a,forward=True,cos=self.tb.config.cos_5)
         ...
         self.localPolicyUdks.policy_set_rule_ipsourcesocket(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.src_ip_a,forward=True,cos=self.tb.config.cos_4)
         self.localPolicyUdks.policy_verify_rule_ipsourcesocket_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.src_ip_a,forward=True,cos=self.tb.config.cos_4)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
