...
class CosRulePriorityTaggedTcpTciOverwriteTests(PolicyBase):
     @mark.EXOS
     @mark.P4
     def test_05_TcpSourcePortIP_CoS_Policy_Rule_QinQ_Priority_Tagged_TCP_Packet_with_TCI_Overwrite(self):
         '''[Documentation]  Test_Objective: Verify_tcpsourceportip_policy_rules_associate_traffic_correctly_for_unlearned/learned unicast, broadcast_and_multicast_QinQ_frames_when_TCI-Overwrite_is_enabled.'''
         ...
         self.localPolicyUdks.Transmit_100_QinQ_Tagged_TCP_Frames_and_Verify_Priority(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_c,self.tb.config.dst_mac_a,self.tb.config.vlan_list_b,self.tb.config.vlan_list_a,self.tb.config.prio_list_a,self.tb.config.prio_list_d,self.tb.config.tpid_list_a,self.tb.config.tpid_list_a,self.tb.config.src_ip_a,self.tb.config.dst_ip_a,self.tb.config.l4_port_b,self.tb.config.l4_port_c)
