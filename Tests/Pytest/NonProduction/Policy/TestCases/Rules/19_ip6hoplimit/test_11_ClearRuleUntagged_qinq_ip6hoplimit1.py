...
class ClearRuleUntaggedTests(PolicyBase):
     ...
     def test_11_IpTTL_Policy_Clear_Drop_Rule_Untagged_Packet(self):
         ...
         self.localPolicyUdks.Remove_Policy_Rule_IpTTL(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.ttl_a)
         self.localPolicyUdks.policy_verify_rule_ipttl_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.ttl_a,drop=True)
         time.sleep(int(self.tb.config.policy_delay))
