...
class ClearRuleUntaggedUdpTests(PolicyBase):
     ...
     def test_11_UdpSourcePortIP_Policy_Clear_Drop_Rule_Untagged_IPv6_UDP_Packet(self):
         '''[Documentation]  Test_Objective: Verify_IPv6_traffic_passes_again_after_removing_an_udpsourceportip_policy_drop_rule.'''
         ...
         self.localPolicyUdks.Remove_Policy_Rule_UdpSourcePortIP(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_port_a)
         self.localPolicyUdks.policy_verify_rule_udpsourceportip_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_port_a,drop=True)
         time.sleep(int(self.tb.config.policy_delay))
