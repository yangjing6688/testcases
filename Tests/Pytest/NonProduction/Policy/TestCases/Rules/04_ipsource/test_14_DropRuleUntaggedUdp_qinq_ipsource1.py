...
class DropRuleUntaggedUdpTests(PolicyBase):
     ...
     def test_14_IpSource_Policy_Drop_Rule_Untagged_UDP_Packet(self):
         '''[Documentation]  Test_Objective: Verify_ipsource_policy_drop_rule_discards_untagged_packets.'''
         ...
         self.localPolicyUdks.policy_set_rule_ipsourcesocket(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.src_ip_b,drop=True)
         ...
         self.localPolicyUdks.policy_verify_rule_ipsourcesocket_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.src_ip_b,drop=True)
