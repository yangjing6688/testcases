...
class ClearRuleUntaggedTests(PolicyBase):
     ...
     def test_11_EtherType_Policy_Clear_Drop_Rule_Untagged_Packet(self):
         ...
         self.localPolicyUdks.Remove_Policy_Rule_EtherType(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.type_ipv4)
         self.localPolicyUdks.policy_verify_rule_ethertype_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.type_ipv4,drop=True)
         time.sleep(int(self.tb.config.policy_delay))
