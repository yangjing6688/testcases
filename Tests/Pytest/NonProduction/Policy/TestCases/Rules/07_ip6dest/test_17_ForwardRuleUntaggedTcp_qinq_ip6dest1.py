...
class ForwardRuleUntaggedTcpTests(PolicyBase):
     ...
     def test_17_Ip6Dest_Policy_Forward_Rule_with_PVID_Override_0_Untagged_TCP_Packet(self):
         '''[Documentation]  Test_Objective: Verify_ip6dest_policy_forward_rule_forwards_untagged_TCP_packets.'''
         ...
         self.localPolicyUdks.policy_set_rule_ip6dest(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_ip6_a,forward=True)
         self.localPolicyUdks.policy_verify_rule_ip6dest_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_ip6_a,forward=True)
