...
class CosRulePriorityTaggedTciOverwriteTests(PolicyBase):
     ...
     def test_05_MacDest_CoS_Policy_Rule_Priority_Tagged_Packet_with_TCI_Overwrite(self):
         '''[Documentation]  Test_Objective: Verify_macdest_policy_rules_associate_traffic_correctly_for_unlearned/learned unicast, broadcast_and_multicast_frames_when_TCI-Overwrite_is_enabled.'''
         ...
         self.localPolicyUdks.Transmit_100_Priority_Tagged_Frames_and_Verify_Priority_Changed(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_b,self.tb.config.vlan_a,self.tb.config.priority_2,self.tb.config.priority_3)
