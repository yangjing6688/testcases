...
class ClearRuleUntaggedTests(PolicyBase):
     ...
     def test_11_Port_Policy_Clear_Drop_Rule_Untagged_Packet(self):
         ...
         self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_NOT_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a)
         ...
         self.localPolicyUdks.policy_verify_rule_port_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.netelem1.tgen.port_a.ifname,drop=True)
         time.sleep(int(self.tb.config.policy_delay))
         self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a)
         ...
         self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_NOT_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.bcast_mac,self.tb.config.vlan_a)
         print('  (Verification_6) Clear_drop_rule_and_Verify_100_unlearned_frames_were_sent_and_received.')
         self.localPolicyUdks.Remove_Policy_Rule_Port(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.netelem1.tgen.port_a.ifname)
         self.localPolicyUdks.policy_verify_rule_port_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.netelem1.tgen.port_a.ifname,drop=True)
         time.sleep(int(self.tb.config.policy_delay))
         self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.bcast_mac,self.tb.config.vlan_a)
         print('  (Step_5) Re-create_policy_drop_rule.')
         self.localPolicyUdks.policy_set_rule_port(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.netelem1.tgen.port_a.ifname,drop=True)
         self.localPolicyUdks.policy_verify_rule_port_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.netelem1.tgen.port_a.ifname,drop=True)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_7) Verify_100_unlearned_frames_were_sent_and_NOT_received.')
         self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_NOT_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.mcast_mac,self.tb.config.vlan_a)
         print('  (Verification_8) Clear_drop_rule_and_Verify_100_unlearned_frames_were_sent_and_received.')
         self.localPolicyUdks.Remove_Policy_Rule_Port(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.netelem1.tgen.port_a.ifname)
         self.localPolicyUdks.policy_verify_rule_port_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.netelem1.tgen.port_a.ifname,drop=True)
         time.sleep(int(self.tb.config.policy_delay))
         self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.mcast_mac,self.tb.config.vlan_a)
