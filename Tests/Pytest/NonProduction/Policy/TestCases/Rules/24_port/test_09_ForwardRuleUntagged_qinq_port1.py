...
class ForwardRuleUntaggedTests(PolicyBase):
     @mark.F_1000043139
     @mark.EXOS
     @mark.P3
     @mark.Policy_Rules
     @mark.Policy_Port_Rules
     def test_09_Port_Policy_Forward_Rule_with_PVID_Override_0_Untagged_Packet(self):
         '''[Documentation]  Test_Objective: Verify_macsource_policy_drop_rule_discards_untagged_packets.'''
         ...
         self.localPolicyUdks.policyUdks.Create_Port_Admin_Profile_and_Verify_it_Exists(self.tb.config.netelem1.name,self.tb.config.netelem1.tgen.port_c.ifname,self.tb.config.profile_a)
         self.localPolicyUdks.policy_set_rule_port(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.netelem1.tgen.port_a.ifname,forward=True)
         self.localPolicyUdks.policy_verify_rule_port_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.netelem1.tgen.port_a.ifname,forward=True)
         ...
         self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_NOT_Received(self.tb.config.tgen_ports.netelem1.port_c,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_b,self.tb.config.dst_mac_a,self.tb.config.vlan_a)
         ...
         self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_NOT_Received(self.tb.config.tgen_ports.netelem1.port_c,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_b,self.tb.config.bcast_mac,self.tb.config.vlan_a)
         ...
         self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_NOT_Received(self.tb.config.tgen_ports.netelem1.port_c,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_b,self.tb.config.mcast_mac,self.tb.config.vlan_a)
