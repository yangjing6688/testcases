...
from Tests.Pytest.Libraries.Common.PlatformGenericSuiteUdks import PlatformGenericSuiteUdks
from Tests.Pytest.Libraries.Common.PolicySuiteUdks import PolicySuiteUdks
from Tests.Pytest.Libraries.Common.RadiusSuiteUdks import RadiusSuiteUdks
from Tests.Pytest.Libraries.Common.TrafficSuiteUdks import TrafficGenerationSuiteUdks
...
class Policy_Test_Suite_Udks():
    ...
    def Initialize_Policy_EMS_Config(self):
        ...
        ERROR_LIST = []
        self.networkElementCliSend.send_cmd(self.netElem1Name, 'enable log debug-mode',  IgnoreError=True)
    ...
    def Test_Suite_Setup(self,):
        ...
        ConfidenceLevel = self.pytestConfigHelper.config.defaultConfidenceLevel
        should_upgrade_firmware = False
        
        if self.pytestConfigHelper.config.upgrade_firmware == 'True':
            should_upgrade_firmware = True
    ...
    def Policy_Test_Case_Setup_Dacl(self, netelem):

        print('  (DACL setup step_1) Disable policy, configure slices and rule type.')
        self.networkElementCliSend.send_cmd(netelem.name, ' disable clipaging')
        self.policyUdks.Disable_Policy_and_Verify_it_is_Disabled(netelem.name)
        self.networkElementCliSend.send_cmd(netelem.name,  'configure policy slices tci-overwrite 1')
        self.networkElementCliSend.send_cmd(netelem.name,  'configure policy slices shared 1')
        self.change_policy_rule_model(netelem.name, 'Access-List')
        self.networkElementPolicyGenKeywords.policy_set_maptable_response(netelem.name, 'policy')

        print('  (DACL setup step_2) Enable user ports')
        self.networkElementPortGenKeywords.port_enable_state(netelem.name, netelem.tgen.port_a.ifname)
        self.networkElementPortGenKeywords.port_enable_state(netelem.name, netelem.tgen.port_b.ifname)

        print('  (DACL setup step_3) Globally enable radius and policy')
        self.radiusSuiteUdks.Globally_Enable_Radius_and_Verify_it_is_Enabled(netelem.name)
        self.policyUdks.Enable_Policy_and_Verify_it_is_Enabled(netelem.name)

        print('  (DACL setup step_4) Globally enable Mac Auth')
        self.macAuthUdks.Enable_Macauth_and_Verify_it_is_Enabled(netelem.name)

        print('  (DACL setup step_5) Enable Mac Auth on user ports')
        self.macAuthUdks.Enable_Macauth_Port_and_Verify_it_is_Enabled( netelem.name, netelem.tgen.port_a.ifname)
        self.macAuthUdks.Enable_Macauth_Port_and_Verify_it_is_Enabled( netelem.name, netelem.tgen.port_b.ifname)

        self.radiusSuiteUdks.configure_netlogin_mac_username_format(netelem.name, 'hyphenated')

    def Policy_Test_Case_Teardown_Dacl(self, netelem):

        print('  (DACL teardown step_1) Reset rule model and disable policy')
        self.change_policy_rule_model(netelem.name, 'Hierarchical')
        self.policyUdks.Disable_Policy_and_Verify_it_is_Disabled(netelem.name)

        print('  (DACL teardown step_2) Set slices back to default settings')
        self.networkElementCliSend.send_cmd(netelem.name,  'configure policy slices shared 0')
        self.networkElementCliSend.send_cmd(netelem.name,  'configure policy slices tci-overwrite 4')

        print('  (DACL teardown step_3) Disable mac auth globally and on ingress ports')
        self.macAuthUdks.Disable_Macauth_and_Verify_it_is_Disabled(netelem.name)
        self.macAuthUdks.Disable_Macauth_Port_and_Verify_it_is_Disabled(netelem.name,
                                                                        netelem.tgen.port_a.ifname)
        self.macAuthUdks.Disable_Macauth_Port_and_Verify_it_is_Disabled(netelem.name,
                                                                        netelem.tgen.port_b.ifname)
        print('  (DACL teardown step_4) Disable radius and reset user name format')
        self.radiusSuiteUdks.Globally_Disable_Radius_and_Verify_it_is_Disabled(netelem.name)
        self.radiusSuiteUdks.configure_netlogin_mac_username_format(netelem.name, 'none')
    ...
    def Create_Policy_Profile_with_Name_and_PVID(self, netelem_name,  profile_id,  profile_name, pvid):
        self.policyUdks.Create_Policy_Profile_with_Name_and_Verify_it_Exists(netelem_name, profile_id, profile_name)
        self.networkElementPolicyGenKeywords.policy_set_profile_pvid(netelem_name,profile_id, pvid)
        self.networkElementPolicyGenKeywords.policy_set_profile_pvid_status(netelem_name,profile_id, 'enable')
        self.networkElementPolicyGenKeywords.policy_verify_profile_pvid(netelem_name, profile_id, pvid)
        self.networkElementPolicyGenKeywords.policy_verify_profile_pvid_status_enabled(netelem_name, profile_id)

    def Create_Policy_Profile_with_Name_PVID_and_ACL(self, netelem_name,  profile_id,  profile_name, acl_name, pvid):
        self.policyUdks.Create_Policy_Profile_with_Name_and_Verify_it_Exists(netelem_name, profile_id, profile_name)
        self.networkElementPolicyGenKeywords.policy_set_profile_pvid(netelem_name,profile_id, pvid)
        self.networkElementPolicyGenKeywords.policy_set_profile_pvid_status(netelem_name,profile_id, 'enable')
        self.networkElementPolicyGenKeywords.policy_verify_profile_pvid(netelem_name, profile_id, pvid)
        self.networkElementPolicyGenKeywords.policy_verify_profile_pvid_status_enabled(netelem_name, profile_id)
        self.networkElementPolicyGenKeywords.policy_set_access_list_profile_index(netelem_name, profile_id, acl_name)

    def Create_Policy_Profile_with_Name_and_ACL(self, netelem_name,  profile_id,  profile_name, acl_name):
        self.policyUdks.Create_Policy_Profile_with_Name_and_Verify_it_Exists(netelem_name, profile_id, profile_name)
        self.networkElementPolicyGenKeywords.policy_set_access_list_profile_index(netelem_name, profile_id, acl_name)
    ...
    def Transmit_and_Verify_Frames_Received_and_Mirrored(self, tx_port,  rx_port,  mirror_port, sentdmac = None,  sentsmac = None ):       
        if sentdmac == None:
            sentdmac = self.pytestConfigHelper.config.dst_mac_a
        if sentsmac == None:
            sentsmac = self.pytestConfigHelper.config.src_mac_a
        ...
        #start capture
        self.tgenUdks.Start_Capture_with_DMAC_and_SMAC_Filter(rx_port, sentdmac, sentsmac)
        self.tgenUdks.Start_Capture_with_DMAC_and_SMAC_Filter(mirror_port, sentdmac, sentsmac)
        #start transmit
        self.trafficTransmitKeywords.start_transmit_on_port(tx_port)
        #wait 
        time.sleep(2)
        ...
        #check stats values
        self.trafficStatisticsKeywords.stat_value_should_be_plus_or_minus(rx_port, '100', '5')
        self.trafficStatisticsKeywords.stat_value_should_be_plus_or_minus(mirror_port, '100', '5')        
        #Get_Tx_Count                            tx_port
        self.trafficStatisticsKeywords.get_tx_count(tx_port) 
        # Stat_Value_Should_be_Equal              tx_port  100           
        self.trafficStatisticsKeywords.stat_value_should_be_equal(tx_port, '100')        
        ...
        self.trafficPacketInspectionKeywords.capture_inspection_range(rx_port,'genericMirror','50','54' )
    ...
    def Transmit_and_Verify_Frames_NOT_Received(self, tx_port, rx_port, sentdmac = None,  sentsmac = None):
        ...
        #Clear_Port_Statistics                   tx_port
        self.trafficCaptureKeywords.clear_port_statistics(tx_port)
        self.trafficCaptureKeywords.clear_port_statistics(rx_port)               
        #Transmit_Packet_on_Port, Single_Burst   tx_port  tx_packet        
        #self.tgenUdks.Create_Ethernet2_Packet('genericTx', self.pytestConfigHelper.config.packetA.dst_mac, self.pytestConfigHelper.config.packetA.src_mac, packet_len='80')
        #config burst of 100       
        self.tgenUdks.Configure_Packet_on_Port_Single_Burst(tx_port, 'genericTx', count='100')
    ...
    def Transmit_100_Untagged_TCP_Frames_and_Verify_Received_and_Mirrored(self,tx_port,rx_port,mirror_port,src_mac,dst_mac, vlan_id, sip, dip, src_port, dst_port,
        ...
        self.Transmit_and_Verify_Frames_Received_and_Mirrored(tx_port, rx_port, mirror_port,dst_mac,src_mac)
    ...
    def Transmit_100_Untagged_UDP_Frames_and_Verify_NOT_Received(self,tx_port,rx_port, src_mac,dst_mac,vlan_id,sip,dip,src_port,dst_port,
        ...
        self.trafficCaptureKeywords.clear_port_stats_and_filters(rx_port)     
        self.tgenUdks.Start_Capture_with_DMAC_Filter(rx_port, dst_mac)
        self.Transmit_and_Verify_Frames_NOT_Received(tx_port, rx_port,dst_mac,src_mac)
    ...
    def Transmit_100_Untagged_UDP_Frames_and_Verify_Priority_Changed(self, tx_port, rx_port, src_mac, dst_mac, vlan_id, rx_priority, sip,  dip, src_port, dst_port,  
        ...
        self.tgenUdks.Create_IPv4_UDP_Packet('genericTx', dst_mac, src_mac, dip=dip, sip=sip,ttl=ttl, tos=tos, flags=flags, frag_offset=offset,
                                             src_port=src_port, dst_port=dst_port)
        self.tgenUdks.Create_IPv4_UDP_Packet('genericRx', dst_mac, src_mac,vlan_id,rx_priority,dip=dip, sip=sip,ttl=ttl, tos=tos, flags=flags, frag_offset=offset,
                                             src_port=src_port, dst_port=dst_port)
        self.tgenUdks.Start_Capture_with_DMAC_Filter(rx_port, dst_mac)
        self.Transmit_and_Verify_Frames_Received(tx_port, rx_port,dst_mac,src_mac)
    ...
    def Basic_Policy_Setup(self):
        self.networkElementPolicyGenKeywords.policy_clear_profile(self.pytestConfigHelper.dut1.name, self.pytestConfigHelper.config.policyId_a)
        self.networkElementPolicyGenKeywords.policy_clear_profile(self.pytestConfigHelper.dut1.name, self.pytestConfigHelper.config.policyId_b)
        self.networkElementPolicyGenKeywords.policy_clear_profile(self.pytestConfigHelper.dut1.name, self.pytestConfigHelper.config.policyId_c)
        # Create the policies that we are going to use..
        self.policyUdks.Create_Policy_Profile_With_PVID_and_PVID_Status_Enabled(self.pytestConfigHelper.dut1.name, self.pytestConfigHelper.config.policyId_a, self.pytestConfigHelper.config.vlan_100)
        self.policyUdks.Create_Policy_Profile_With_PVID_and_PVID_Status_Enabled(self.pytestConfigHelper.dut1.name, self.pytestConfigHelper.config.policyId_b, self.pytestConfigHelper.config.vlan_200)
        self.policyUdks.Create_Policy_Profile_With_PVID_and_PVID_Status_Enabled(self.pytestConfigHelper.dut1.name, self.pytestConfigHelper.config.policyId_c, self.pytestConfigHelper.config.vlan_300)
        self.policyUdks.Create_Policy_Profile_with_Name_and_Verify_it_Exists(self.pytestConfigHelper.dut1.name, self.pytestConfigHelper.config.policyId_a, self.pytestConfigHelper.config.policyName_a)
        self.policyUdks.Create_Policy_Profile_with_Name_and_Verify_it_Exists(self.pytestConfigHelper.dut1.name, self.pytestConfigHelper.config.policyId_b, self.pytestConfigHelper.config.policyName_b)
        self.policyUdks.Create_Policy_Profile_with_Name_and_Verify_it_Exists(self.pytestConfigHelper.dut1.name, self.pytestConfigHelper.config.policyId_c, self.pytestConfigHelper.config.policyName_c)
        # Create Port Admin Profile and Verify it Exists  ${netelem1.name}  ${netelem1.tgen.port_a.ifname}  ${policyId_a}
        self.networkElementPolicyGenKeywords.policy_set_maptable_response(self.pytestConfigHelper.dut1.name, 'policy')
        
    def Basic_Policy_Cleanup(self):
        self.policyUdks.Remove_Policy_Profile_and_Verify_it_was_Removed(self.pytestConfigHelper.dut1.name, self.pytestConfigHelper.config.policyId_a)
        self.policyUdks.Remove_Policy_Profile_and_Verify_it_was_Removed(self.pytestConfigHelper.dut1.name, self.pytestConfigHelper.config.policyId_b)
        self.policyUdks.Remove_Policy_Profile_and_Verify_it_was_Removed(self.pytestConfigHelper.dut1.name, self.pytestConfigHelper.config.policyId_c)
