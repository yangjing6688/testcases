...
class SuiteUdk:
    ...
    def verify_path_cost_on_dut(self, dut, port, expected_path_cost, mode="mstp", retries=10, step=60):
        for _ in range(retries):
            ...
            pytest.fail(f"Failed to verify the path cost of port='{port}' on this dut\n{dut}")
    ...
    def verify_path_cost_in_port_configuration_stp_tab(self, template_switch, network_policy, port, path_cost):
    ...
    def verify_path_cost_at_template_level(
        ...
        revert_configuration=True, port_order_in_asic=None, ports=None):
        
        if not default_path_cost:
            default_path_cost = "200000000" if onboarded_switch.cli_type.upper() == "VOSS" else "20000"

        if not ports:
            ports = self.get_one_port_from_each_asic(
                dut=onboarded_switch, order=port_order_in_asic)
        ...
        try:
            ...
            for port, port_type_config in port_config.items():
                ...
                self.verify_path_cost_in_port_configuration_stp_tab(
                    template_switch, network_policy, port,
                    port_type_config["path_cost"]
                )
    ...
    def get_one_port_from_each_asic(self, dut, order):
        self.wait_till(timeout=5)
        self.xiq.xflowscommonDevices._goto_devices()
        ...
        try:
            ...
            return self.dev360.get_one_port_from_each_asic(order=order)
