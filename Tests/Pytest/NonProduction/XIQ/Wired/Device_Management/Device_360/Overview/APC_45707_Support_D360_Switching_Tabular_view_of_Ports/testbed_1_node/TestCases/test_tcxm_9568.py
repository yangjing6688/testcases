...
class TCXM9568Tests(xiqBase):

    @pytest.mark.xim_tcxm_9568
    @pytest.mark.run(order=2)
    @pytest.mark.development
    @pytest.mark.p1
    @pytest.mark.testbed_1_node
    def test_tcxm_9568(self, onboarded_switch):
        """
        Step	Step Description
        1	    Onboard the device
        2	    Navigate to Device360
        3	    Verify the 'TRAFFIC TRANSMITTED(RX)' check mark in the columns section
        4	    Verify the 'TRAFFIC TRANSMITTED(RX)' column displays values for all the ports(the first 10 ports)
        5	    Display and verify the values for 'TRAFFIC TRANSMITTED(RX)' in CLI for the following ports: 1, 24
        6	    Compare the values from XIQ  to the CLI values of 'TRAFFIC TRANSMITTED(RX)' for the following ports: 1, 24
        7	    Navigate to the 100 ports option
        8	    Verify the 'TRAFFIC TRANSMITTED(RX)' column displays values for all the ports(100 ports)
        9	    Display and verify the values for 'TRAFFIC TRANSMITTED(RX)' in CLI for the following ports: 1, 24
        10	    Compare the values from XIQ  to the CLI values of 'TRAFFIC TRANSMITTED(RX)'' for the following ports: 1, 24
        11	    Setup clean-up - Exit Device360 and logout from XIQ
        
        """
        self.executionHelper.testSkipCheck()
        self.cfg['${TEST_NAME}'] = 'test_apc_45707_tcxm_9568'
        ...
        try:
            ...
            self.suite_udk.Transmit_Traffic_Bidirectionally_and_Verify_it_was_Received_tcxm_9568(
            ...
            try:
                ...
                print("Checking if the Transmitted Traffic checkbox is marked in the columns section")
                for i in checkboxes:
                    if "Transmitted" in i:
            ...
            while time.time() - start_time < 1200:
                ...
                try:
                    ...
                    traffic_list_from_dut = self.suite_udk.get_transmitted_traffic_list_from_dut(
                    ...
                    self.suite_udk.go_to_device360(onboarded_switch)
                    traffic_list_from_xiq = self.suite_udk.device360_display_traffic_transmitted_from_xiq_and_return_traffic_list(
                        dut, first_port, second_port)
