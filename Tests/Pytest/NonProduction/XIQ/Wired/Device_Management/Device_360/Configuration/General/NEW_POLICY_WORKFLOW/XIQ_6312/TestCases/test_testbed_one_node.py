...
class XIQ6312OneNodeTests:
    ...
    @pytest.mark.EXOS
    @pytest.mark.VOSS
    def test_23826(self, logger, xiq_library_at_class_level, node_1_policy_name, auto_actions, navigator):
        ...
        logger.info("Clicking 'Switching tab'")
        auto_actions.click_reference(xiq_np.np_web_elements.get_switching_tab)
        assert xiq_library_at_class_level.xflowsconfigureNetworkPolicy.check_port_types_section() == 1, "Port types section not found"
    ...
    def test_23827(self, logger, xiq_library_at_class_level, node_1, node_1_policy_name, node_1_template_name, utils):
        ...
        assert xiq_library_at_class_level.xflowsconfigureNetworkPolicy.create_new_port_type(
            node_1.cli_type.upper()) == 1, "Port type Configuration did not open"

        _, create_port_type_summary = xiq_library_at_class_level.xflowsconfigureNetworkPolicy.configure_port_type(
            port_type_name)
        logger.info(f"Summary from create port type in Port Types Section: {create_port_type_summary}")
        logger.step(f"Verify that the configured fields appear correctly in the summary tab ")
        ...
        xiq_library_at_class_level.xflowsconfigureNetworkPolicy.go_to_specific_tab_in_port_type_configuration(
            tab_name="NAME")
        xiq_library_at_class_level.xflowsmanageDevice360.configure_port_name_usage_tab(port_type_name=port_type_name,
                                                                                       status=False)
        ...
        edit_port_type_summary = xiq_library_at_class_level.xflowsconfigureNetworkPolicy.get_port_type_row_details(
            port_type_name,
        ...
        logger.step("Verify user can delete port-type")
    ...
    def test_23833(self, logger, xiq_library_at_class_level, node_1, node_1_policy_name):
        assert xiq_library_at_class_level.xflowscommonDevices.assign_network_policy_to_switch_mac(
            policy_name=node_1_policy_name, mac=node_1.mac), \
            f"Couldn't assign policy {node_1_policy_name} to device {node_1}"
        logger.info("Checking if configuration is present in Delta CLI")
