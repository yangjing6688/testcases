...
class TCXM20638Tests(xiqBase):
    @pytest.mark.development
    @pytest.mark.testbed_stack
    @pytest.mark.xim_tcxm_20638
    @pytest.mark.p1
    def test_verify_lacp_for_vim_ports_d360(self, onboarded_dut):
        ...
        # Select unit from stack with VIM ports
            ...
            lag_rows = self.xiq.xflowsmanageDevice360.get_device360_configure_aggregated_port_settings_aggregation_rows()
        ...
        # Remove 2 VIM ports
            assert self.suite_udk.device360_remove_partial_lag(fourth_vim_port) == 1, f"LAG {fourth_vim_port} was not removed"
        ...
        # Select unit from stack with VIM ports
            ...
            lacp_ports_d360 = [third_vim_port + "-" + str(ports_no - 1)]
            assert self.suite_udk.check_lacp_dut_cli(lacp_ports_d360), "Incorrect number of LACPs from dut"

        # Remove the remaining VIM ports
            assert self.suite_udk.device360_remove_partial_lag(third_vim_port) == 1, f"LAG {third_vim_port} was not removed"
            time.sleep(5)
