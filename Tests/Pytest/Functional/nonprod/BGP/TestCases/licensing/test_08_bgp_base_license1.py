...
class License08Tests(BgpBase):
     
     @mark.ONE_BOX
     def test_08_BGP_Base_License(self):
         '''[Documentation]  Test Objective: This test is intended to be run on unified hardware running EXOS, version 31.4 or later. Configure a base license only and configure 2 peers one on the default VR and one on a VRF residing under the default VR. Delete 1 peer from VR-Default, verify a second peer can then be configured on the VRF.'''
         ...
         print('  (Step 8) Delete 1 BGP neighbor on default VR')
         self.localsuiteudks.Delete_BGP_Neighbor(self.tb.config.netelem1.name,self.tb.config.globalvars.bgp.peer1)
         print('  (Step 9) Verify the neighbor does not exist on default VR')
         self.localsuiteudks.Verify_BGP_Neighbor_Does_Not_Exist(self.tb.config.netelem1.name,self.tb.config.globalvars.bgp.peer1,self.tb.config.globalvars.bgp.localAS)
         self.localsuiteudks.Verify_BGP_Number_Neighbors(self.tb.config.netelem1.name,self.tb.config.globalvars.bgp.zeroPeers)
         print('  (Step 10) Create a 2nd BGP neighbor on VRF')
         ...
         print('  (Step 11) Verify only the number of neighbors on VRF is 2')
         ...
         self.TestCase8Cleanup()
     def TestCase8Cleanup(self):
         ...
         print('  (Step 13) Disable BGP and reset AS and router ID on vrf')
