...
class UdpDestPortIpv6Tests(PolicyBase):
    @mark.F_129412
    @mark.EXOS
    @mark.P3
    @mark.Policy_Masking
    ...
    def test_13_Masking_Precedence_IPv6_UDPDestPortIP(source):
        ...
        self.localPolicyUdks.policy_set_rule_udpdestportip(           self.tb.config.netelem1.name,  self.tb.config.profile_a,  self.tb.config.mask_l4ip_b,  mask='8',
                                                        drop=True)
        self.localPolicyUdks.policy_verify_rule_udpdestportip_exists(     self.tb.config.netelem1.name,  self.tb.config.profile_a,  self.tb.config.mask_l4ip_a,  mask='8')
        ...
        self.localPolicyUdks.Transmit_100_Untagged_IPv6_UDP_Frames_and_Verify_NOT_Received(
                  ...
                  self.tb.config.src_ip_b,       self.tb.config.src_ip_a,  src_port=self.tb.config.l4_port_b,          dst_port=self.tb.config.l4_port_a)
        ...
        self.localPolicyUdks.policy_set_rule_udpdestportip(        self.tb.config.netelem1.name,  self.tb.config.profile_a,  self.tb.config.mask_l4ip_b,  mask='16',
                                                     forward=True)
        self.localPolicyUdks.policy_verify_rule_udpdestportip_exists(  self.tb.config.netelem1.name,  self.tb.config.profile_a,  self.tb.config.mask_l4ip_b,  mask='16')
        time.sleep(int(self.tb.config.policy_delay))  # Delay_for_EXOS_policy_hardware_config_batching.
