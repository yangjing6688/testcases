...
class IpTosTests(PolicyBase):
    ...
    def test_16_Masking_Precedence_IP_TTL(self):
        '''[Documentation]  Test_Objective: Verify_iptos_policy_rules_drop_or_forward_traffic_correctly_when_using_masks.'''
        ...
        self.localPolicyUdks.policy_set_rule_iptos(self.tb.config.netelem1.name,  self.tb.config.profile_a, self.tb.config.mask_tos_b,  mask='4',
                                                        drop=True)
        self.localPolicyUdks.policy_verify_rule_iptos_exists(self.tb.config.netelem1.name,  self.tb.config.profile_a, self.tb.config.mask_tos_a,  mask='4')
        ...
        self.localPolicyUdks.Transmit_100_Untagged_IP_ToS_Frames_and_Verify_NOT_Received(
                  ...
                  self.tb.config.src_ip_a,       self.tb.config.dst_ip_a,  self.tb.config.mask_tos_b)
    
        print('  (Verification_2) Verify_100_unlearned_frames_were_sent_and_NOT_received.')
        self.localPolicyUdks.Transmit_100_Tagged_IP_ToS_Frames_and_Verify_NOT_Received(
                  self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,  self.tb.config.src_mac_a,  self.tb.config.dst_mac_a,  self.tb.config.vlan_a,
                  self.tb.config.src_ip_a,       self.tb.config.dst_ip_a,  self.tb.config.mask_tos_b)
    
        print('  (Step_3) Create_additional_Policy_Rules.')
        self.localPolicyUdks.networkElementPolicyGenKeywords.policy_set_rule_iptos(self.tb.config.netelem1.name,  self.tb.config.profile_a,  self.tb.config.mask_tos_b,  mask='8', forward=True)
        self.localPolicyUdks.networkElementPolicyGenKeywords.policy_verify_rule_iptos_exists(  self.tb.config.netelem1.name,  self.tb.config.profile_a,  self.tb.config.mask_tos_b,  mask='8')
        time.sleep(int(self.tb.config.policy_delay))  # Delay_for_EXOS_policy_hardware_config_batching.
    
        print('  (Verification_3) Verify_100_unlearned_frames_were_sent_and_received.')
        self.localPolicyUdks.Transmit_100_Untagged_IP_ToS_Frames_and_Verify_Received(
                  self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,  self.tb.config.src_mac_a,  self.tb.config.dst_mac_a,  self.tb.config.vlan_a,
                  self.tb.config.src_ip_a,       self.tb.config.dst_ip_a,  self.tb.config.mask_tos_b)
    
        print('  (Verification_4) Verify_100_unlearned_frames_were_sent_and_received.')
        self.localPolicyUdks.Transmit_100_Tagged_IP_ToS_Frames_and_Verify_Received(
                  self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,  self.tb.config.src_mac_a,  self.tb.config.dst_mac_a,  self.tb.config.vlan_a,
                  self.tb.config.src_ip_a,       self.tb.config.dst_ip_a,  self.tb.config.mask_tos_b)
