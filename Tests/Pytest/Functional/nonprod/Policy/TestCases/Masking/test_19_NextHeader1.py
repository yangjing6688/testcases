...
class NextHeaderTests(PolicyBase):    
    @mark.EXOS
    @mark.P3
    @mark.Policy_Masking
    @mark.NIGHTLY
    
    def test_19_Masking_Precedence_IP_Proto(self):
        ...
        self.localPolicyUdks.Transmit_100_Untagged_IPv6_Next_Header_Frames_and_Verify_NOT_Received(
        ...
        self.localPolicyUdks.Transmit_100_Tagged_IPv6_Next_Header_Frames_and_Verify_NOT_Received(
                  self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,  self.tb.config.src_mac_a,  self.tb.config.dst_mac_a,  self.tb.config.vlan_a,
                  self.tb.config.src_ip_a,       self.tb.config.dst_ip_a,  self.tb.config.mask_proto_b)
        ...
        self.localPolicyUdks.Transmit_100_Untagged_IPv6_Next_Header_Frames_and_Verify_Received(
                  self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,  self.tb.config.src_mac_a,  self.tb.config.dst_mac_a,  self.tb.config.vlan_a,
                  self.tb.config.src_ip_a,       self.tb.config.dst_ip_a,  self.tb.config.mask_proto_b)
    
        print('  (Verification_4) Verify_100_unlearned_frames_were_sent_and_received.')
        self.localPolicyUdks.Transmit_100_Tagged_IPv6_Next_Header_Frames_and_Verify_Received(
                  self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,  self.tb.config.src_mac_a,  self.tb.config.dst_mac_a,  self.tb.config.vlan_a,
                  self.tb.config.src_ip_a,       self.tb.config.dst_ip_a,  self.tb.config.mask_proto_b)
