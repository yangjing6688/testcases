...
class IpDestVsIpFragTests(PolicyBase):
     @mark.EXOS
     @mark.P3
     @mark.Precedence
     @mark.Precedence_IpV6Tcp
     @mark.NIGHTLY
     def test_07_IPv6_TCP_Precedence_IPv6_Destination_vs_IPv6_Fragment(self):
         ...
         self.localPolicyUdks.policy_set_rule_ipfrag(self.tb.config.netelem1.name,self.tb.config.profile_a,forward=True,cos=self.tb.config.cos_5)
         ...
         self.localPolicyUdks.Transmit_100_Untagged_IPv6_TCP_Fragments_and_Verify_Priority_Changed(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.priority_5,self.tb.config.src_ip6_a,self.tb.config.dst_ip6_a,src_port=self.tb.config.l4_port_a,dst_port=self.tb.config.l4_port_c)
         print('  (Step_3) Create_policy_profile, rules, and_admin_port_rule.')
         self.localPolicyUdks.policy_set_rule_ip6dest(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_ip6_a,drop=True)
         self.localPolicyUdks.policy_verify_rule_ip6dest_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_ip6_a)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
         self.localPolicyUdks.Transmit_100_Untagged_IPv6_TCP_Fragments_and_Verify_NOT_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.src_ip6_a,self.tb.config.dst_ip6_a,src_port=self.tb.config.l4_port_a,dst_port=self.tb.config.l4_port_c)
         print('  (Step_4) Create_policy_profile, rules, and_admin_port_rule.')
         self.localPolicyUdks.networkElementPolicyGenKeywords.policy_set_rule_ip6dest(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_ip6_a,forward=True,cos=self.tb.config.cos_4)
         self.localPolicyUdks.networkElementPolicyGenKeywords.policy_verify_rule_ip6dest_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_ip6_a,cos=self.tb.config.cos_4)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
