...
class HopLimitVsTrafficClassTests(PolicyBase):
     @mark.F_1000043245
     @mark.EXOS
     @mark.P2
     @mark.Precedence
     ...
     @mark.NIGHTLY
     def test_04_IPv6_ICMP_Precedence_Hop_Limit_vs_Traffic_Class(self):
         '''[Documentation]  Test_Objective: Verify_TTL_rule_takes_precedence_over_iptos_rule.'''
         ...
         self.localPolicyUdks.policy_set_rule_iptos(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.tos_a,forward=True,cos=self.tb.config.cos_5)
         self.localPolicyUdks.policy_verify_rule_iptos_exist(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.tos_a,cos=self.tb.config.cos_5)
         ...
         self.localPolicyUdks.Transmit_100_Untagged_IPv6_ICMP_Frames_and_Verify_Priority_Changed(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.priority_5,self.tb.config.src_ip6_a,self.tb.config.dst_ip6_a,hop_limit=self.tb.config.ttl_a,traffic_class=self.tb.config.tos_a)
         print('  (Step_3) Create_policy_profile, rules, and_admin_port_rule.')
         self.localPolicyUdks.policy_set_rule_ipttl(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.ttl_a,drop=True)
         self.localPolicyUdks.policy_verify_rule_ipttl_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.ttl_a,drop=True)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
         self.localPolicyUdks.Transmit_100_Untagged_IPv6_ICMP_Frames_and_Verify_NOT_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.src_ip6_a,self.tb.config.dst_ip6_a,hop_limit=self.tb.config.ttl_a,traffic_class=self.tb.config.tos_a)
         print('  (Step_4) Create_policy_profile, rules, and_admin_port_rule.')
         self.localPolicyUdks.policy_set_rule_ipttl(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.ttl_a,forward=True,cos=self.tb.config.cos_4)
         self.localPolicyUdks.policy_verify_rule_ipttl_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.ttl_a,cos=self.tb.config.cos_4)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
         self.localPolicyUdks.Transmit_100_Untagged_IPv6_ICMP_Frames_and_Verify_Priority_Changed(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.priority_4,self.tb.config.src_ip6_a,self.tb.config.dst_ip6_a,hop_limit=self.tb.config.ttl_a,traffic_class=self.tb.config.tos_a)
