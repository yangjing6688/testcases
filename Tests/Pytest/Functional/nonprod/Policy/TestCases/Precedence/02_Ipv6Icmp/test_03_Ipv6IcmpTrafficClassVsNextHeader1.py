...
class TrafficClassVsNextHeaderTests(PolicyBase):
     ...
     @mark.NIGHTLY
     def test_03_IPv6_ICMP_Precedence_Traffic_Class_vs_NextHeader(self):
         ...
         self.localPolicyUdks.policy_set_rule_ipproto(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.proto_icmp6,forward=True,cos=self.tb.config.cos_5)
         self.localPolicyUdks.policy_verify_rule_ipproto_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.proto_icmp6,forward=True,cos=self.tb.config.cos_5)
