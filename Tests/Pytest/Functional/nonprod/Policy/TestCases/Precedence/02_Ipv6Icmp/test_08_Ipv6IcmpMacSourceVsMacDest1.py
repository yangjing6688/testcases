...
class MacSourceVsMacDestTests(PolicyBase):
     @mark.F_1000043249
     @mark.EXOS
     @mark.P2
     @mark.Precedence
     ...
     @mark.NIGHTLY
     def test_08_IPv6_ICMP_Precedence_MAC_Source_vs_MAC_Dest(self):
         ...
         self.localPolicyUdks.policy_set_rule_macdest(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_mac_a,forward=True,cos=self.tb.config.cos_5)
         self.localPolicyUdks.policy_verify_rule_macdest_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_mac_a,forward=True,cos=self.tb.config.cos_5)
         ...
         self.localPolicyUdks.policy_set_rule_macsource(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.src_mac_a,forward=True,cos=self.tb.config.cos_4)
         self.localPolicyUdks.policy_verify_rule_macsource_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.src_mac_a,forward=True,cos=self.tb.config.cos_4)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
