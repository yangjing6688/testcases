...
class MacDestVsIpSourceSocketTests(PolicyBase):
     ...
     @mark.NIGHTLY
     def test_13_IPv4_TCP_Precedence_MAC_Dest_vs_IP_Source(self):
         ...
         self.localPolicyUdks.policy_set_rule_ipsourcesocket(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.src_socket,cos=self.tb.config.cos_5)
         self.localPolicyUdks.policy_verify_rule_ipsourcesocket_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.src_socket,forward=True,cos=self.tb.config.cos_5)
