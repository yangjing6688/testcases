...
class UdpSourcePortIpVsUDpDestPortTests(PolicyBase):
     @mark.F_1000043297
     @mark.EXOS
     @mark.P1
     @mark.Precedence
     @mark.Precedence_IpV4Udp
     @mark.NIGHTLY
     def test_06_IPv4_UDP_Precedence_UDP_Source_Port_vs_UDP_Dest_Port(self):
         ...
         self.localPolicyUdks.policy_set_rule_udpdestportip(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_dst_socket,forward=True,cos=self.tb.config.cos_5)
         self.localPolicyUdks.policy_verify_rule_udpdestportip_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_dst_socket,cos=self.tb.config.cos_5)
         ...
         self.localPolicyUdks.policy_verify_rule_udpsourceportip_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_port_a)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
         self.localPolicyUdks.Transmit_100_Untagged_UDP_Frames_and_Verify_NOT_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.src_ip_a,self.tb.config.dst_ip_a,ttl=self.tb.config.ttl_a,tos=self.tb.config.tos_a,src_port=self.tb.config.l4_port_a,dst_port=self.tb.config.l4_port_c)
         print('  (Step_4) Create_policy_profile, rules, and_admin_port_rule.')
         self.localPolicyUdks.policy_set_rule_udpsourceportip(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_port_a,forward=True,cos=self.tb.config.cos_4)
         self.localPolicyUdks.policy_verify_rule_udpsourceportip_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_port_a,cos=self.tb.config.cos_4)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
         self.localPolicyUdks.Transmit_100_Untagged_UDP_Frames_and_Verify_Priority_Changed(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.priority_4,self.tb.config.src_ip_a,self.tb.config.dst_ip_a,ttl=self.tb.config.ttl_a,tos=self.tb.config.tos_a,src_port=self.tb.config.l4_port_a,dst_port=self.tb.config.l4_port_c)
