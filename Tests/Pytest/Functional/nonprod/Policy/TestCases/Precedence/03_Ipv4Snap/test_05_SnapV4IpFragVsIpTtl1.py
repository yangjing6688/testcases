...
class IpFragVsIpTtlTests(PolicyBase):
     @mark.EXOS
     @mark.P3
     @mark.Precedence
     @mark.Precedence_IpV4Snap
     @mark.NIGHTLY
     def test_05_IPv4_SNAP_Precedence_IP_Fragment_vs_IP_TTL(self):
         ...
         self.localPolicyUdks.policy_verify_rule_ipttl_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.ttl_a,forward=True,cos=self.tb.config.cos_5)
         ...
         self.localPolicyUdks.policy_verify_rule_ipfrag_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,forward=True,cos=self.tb.config.cos_4)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
         self.localPolicyUdks.Transmit_100_Untagged_IPv4_SNAP_Frames_and_Verify_Priority_Changed(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.priority_4,self.tb.config.src_ip_a,self.tb.config.dst_ip_a,ttl=self.tb.config.ttl_a,flags=self.tb.config.mf_flag,offset=self.tb.config.pkt_offset)
