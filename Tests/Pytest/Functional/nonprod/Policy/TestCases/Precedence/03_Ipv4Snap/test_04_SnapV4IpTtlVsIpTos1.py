...
class IpTtlVsIpTosTests(PolicyBase):
     ...
     @mark.P3
     @mark.Precedence
     @mark.Precedence_IpV4Snap
     @mark.NIGHTLY
     def test_04_IPv4_SNAP_Precedence_IP_TTL_vs_IP_ToS(self):
         ...
         self.localPolicyUdks.policy_verify_rule_iptos_exist(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.tos_a,forward=True,cos=self.tb.config.cos_5)
         ...
         self.localPolicyUdks.Transmit_100_Untagged_IPv4_SNAP_Frames_and_Verify_Priority_Changed(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.priority_5,self.tb.config.src_ip_a,self.tb.config.dst_ip_a,ttl=self.tb.config.ttl_a,tos=self.tb.config.tos_a)
         ...
         self.localPolicyUdks.Transmit_100_Untagged_IPv4_SNAP_Frames_and_Verify_NOT_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.src_ip_a,self.tb.config.dst_ip_a,ttl=self.tb.config.ttl_a,tos=self.tb.config.tos_a)
         ...
         self.localPolicyUdks.policy_verify_rule_ipttl_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.ttl_a,forward=True,cos=self.tb.config.cos_4)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
         self.localPolicyUdks.Transmit_100_Untagged_IPv4_SNAP_Frames_and_Verify_Priority_Changed(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.priority_4,self.tb.config.src_ip_a,self.tb.config.dst_ip_a,ttl=self.tb.config.ttl_a,tos=self.tb.config.tos_a)
