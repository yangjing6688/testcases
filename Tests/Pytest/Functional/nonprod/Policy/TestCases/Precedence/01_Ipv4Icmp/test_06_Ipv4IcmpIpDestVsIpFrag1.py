...
class IpDestVsIpFragTests(PolicyBase):
     ...
     @mark.NIGHTLY
     def test_06_IPv4_ICMP_Precedence_IP_Destination_vs_IP_Fragment(self):
         '''[Documentation]  Test_Objective: Verify_ipdestsocket_takes_precedence_over_ipfrag_rule.'''
         ...
         self.localPolicyUdks.policy_verify_rule_ipfrag_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,forward=True,cos=self.tb.config.cos_5)
         ...
         self.localPolicyUdks.Transmit_100_Untagged_IPv4_ICMP_Frames_and_Verify_NOT_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.src_ip_a,self.tb.config.dst_ip_a,flags=self.tb.config.mf_flag,offset=self.tb.config.pkt_offset)
         print('  (Step_4) Create_policy_profile, rules, and_admin_port_rule.')
         self.localPolicyUdks.policy_set_rule_ipdestsocket(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_ip_a,forward=True,cos=self.tb.config.cos_4)
         self.localPolicyUdks.policy_verify_rule_ipdestsocket_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_ip_a,forward=True,cos=self.tb.config.cos_4)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
         self.localPolicyUdks.Transmit_100_Untagged_IPv4_ICMP_Frames_and_Verify_Priority_Changed(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.priority_4,self.tb.config.src_ip_a,self.tb.config.dst_ip_a,flags=self.tb.config.mf_flag,offset=self.tb.config.pkt_offset)
