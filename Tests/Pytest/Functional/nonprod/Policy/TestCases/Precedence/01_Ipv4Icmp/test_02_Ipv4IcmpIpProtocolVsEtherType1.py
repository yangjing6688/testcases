...
class IpProtocolVsEtherTypeTests(PolicyBase):
     ...
     @mark.NIGHTLY
     def test_02_IPv4_ICMP_Precedence_IP_Protocol_vs_EtherType(self):
         ...
         self.localPolicyUdks.policy_verify_rule_ether_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.type_ipv4,cos=self.tb.config.cos_5)
         ...
         self.localPolicyUdks.policy_set_rule_ipproto(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.proto_icmp,drop=True)
         self.localPolicyUdks.policy_verify_rule_ether_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.proto_icmp)
         time.sleep(int(self.tb.config.policy_delay))
         ...
         self.localPolicyUdks.policy_set_rule_ipproto(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.proto_icmp,forward=True,cos=self.tb.config.cos_4)
         self.localPolicyUdks.policy_verify_rule_ether_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.proto_icmp,cos=self.tb.config.cos_4)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_2) Verify_100_frames_were_sent_and_NOT_received.')
