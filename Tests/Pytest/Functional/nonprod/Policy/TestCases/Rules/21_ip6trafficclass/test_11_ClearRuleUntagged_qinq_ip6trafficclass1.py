...
class ClearRuleUntaggedTests(PolicyBase):
     ...
     def test_11_Ip6TrafficClass_Policy_Clear_Drop_Rule_Untagged_Packet(self):
         ...
         self.localPolicyUdks.Remove_Policy_Rule_IpToS(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.class_a)
         self.localPolicyUdks.policy_verify_rule_iptos_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.class_a,drop=True)
         time.sleep(int(self.tb.config.policy_delay))
