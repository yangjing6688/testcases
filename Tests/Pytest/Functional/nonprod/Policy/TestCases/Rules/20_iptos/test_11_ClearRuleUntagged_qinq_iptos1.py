...
class ClearRuleUntaggedTests(PolicyBase):
     ...
     def test_11_IPToS_Policy_Clear_Drop_Rule_Untagged_Packet(self):
         '''[Documentation]  Test_Objective: Verify_traffic_passes_again_after_removing_an_iptos_policy_drop_rule.'''
         ...
         self.localPolicyUdks.Remove_Policy_Rule_IPToS(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.tos_a)
         self.localPolicyUdks.policy_verify_rule_iptos_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.tos_a,drop=True)
         time.sleep(int(self.tb.config.policy_delay))
