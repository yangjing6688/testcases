...
class ForwardRuleTaggedTests(PolicyBase):
     ...
     def test_10_IpTTL_Policy_Forward_Rule_with_PVID_Override_0_Tagged_Packet(self):
         ...
         self.localPolicyUdks.policy_set_rule_ipttl(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.ttl_a,forward=True)
         self.localPolicyUdks.policy_verify_rule_ipttl_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.ttl_a,forward=True)
         time.sleep(int(self.tb.config.policy_delay))
         print('  (Verification_1) Verify_100_unlearned_frames_were_sent_and_received.')
         self.localPolicyUdks.Transmit_100_Tagged_IP_TTL_Frames_and_Verify_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.src_ip_a,self.tb.config.dst_ip_a,self.tb.config.ttl_a)
         ...
         self.localPolicyUdks.Transmit_100_Tagged_IP_TTL_Frames_and_Verify_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.bcast_mac,self.tb.config.vlan_a,self.tb.config.src_ip_a,self.tb.config.dst_ip_a,self.tb.config.ttl_a)
         ...
         self.localPolicyUdks.Transmit_100_Tagged_IP_TTL_Frames_and_Verify_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.mcast_mac,self.tb.config.vlan_a,self.tb.config.src_ip_a,self.tb.config.dst_ip_a,self.tb.config.ttl_a)
