...
class ClearRuleUntaggedUdpTests(PolicyBase):
     @mark.F_1000043043
     @mark.EXOS
     @mark.P3
     @mark.Policy_Rules
     @mark.Policy_UdpDstPortIpV6_Rules
     def test_11_UdpDestPortIP_Policy_Clear_Drop_Rule_Untagged_IPv6_UDP_Packet(self):
         '''[Documentation]  Test_Objective: Verify_traffic_passes_again_after_removing_an_udpdestportip_policy_drop_rule.'''
         ...
         self.localPolicyUdks.Remove_Policy_Rule_UdpDestPortIP(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_port_c)
         self.localPolicyUdks.policy_verify_rule_udpdestportip_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.l4_port_c,drop=True)
         time.sleep(int(self.tb.config.policy_delay))
         ...
         self.localPolicyUdks.Transmit_100_Untagged_IPv6_UDP_Frames_and_Verify_NOT_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.src_ip6_a,self.tb.config.dst_ip6_a,self.tb.config.l4_port_a,self.tb.config.l4_port_c,src_port=self.tb.config.l4_port_a,dst_port=self.tb.config.l4_port_c)
