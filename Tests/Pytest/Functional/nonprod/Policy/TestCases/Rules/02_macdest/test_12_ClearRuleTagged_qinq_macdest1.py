...
class ClearRuleTaggedTests(PolicyBase):
     ...
     def test_12_MacDest_Policy_Clear_Drop_Rule_Tagged_Packet(self):
         '''[Documentation]  Test_Objective: Verify_traffic_passes_again_after_removing_a_macdest_policy_drop_rule.'''
         ...
         self.localPolicyUdks.Remove_Policy_Rule_MacDest(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_mac_a)
         self.localPolicyUdks.policy_verify_rule_macdest_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_mac_a,drop=True)
         time.sleep(int(self.tb.config.policy_delay))
