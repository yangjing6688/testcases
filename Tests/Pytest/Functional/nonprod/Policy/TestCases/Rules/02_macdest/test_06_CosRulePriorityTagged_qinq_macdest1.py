...
class CosRulePriorityTaggedTests(PolicyBase):
     ...
     @mark.P3
     @mark.Policy_Rules
     @mark.Policy_MacDst_Rules
     def test_06_MacDest_CoS_Policy_Rule_Priority_Tagged_Packet_without_TCI_Overwrite(self):
         '''[Documentation]  Test_Objective: Verify_macdest_policy_rules_associate_traffic_correctly_for_unlearned/learned unicast, broadcast_and_multicast_frames_when_TCI-Overwrite_is_disabled.'''
         ...
         self.localPolicyUdks.policy_set_rule_macdest(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_mac_a,cos=self.tb.config.cos_5)
         self.localPolicyUdks.policy_verify_rule_macdest_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_mac_a,cos=self.tb.config.cos_5)
         ...
         self.localPolicyUdks.Transmit_100_Tagged_Frames_and_Verify_Priority_has_NOT_Changed(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_b,self.tb.config.vlan_a,self.tb.config.priority_2)
         ...
         self.localPolicyUdks.Transmit_100_Priority_Tagged_Frames_and_Verify_Priority_has_NOT_Changed(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_b,self.tb.config.vlan_a,self.tb.config.priority_2)
