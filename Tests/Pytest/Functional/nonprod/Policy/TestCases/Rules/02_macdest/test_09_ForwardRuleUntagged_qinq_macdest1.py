...
class ForwardRuleUntaggedTests(PolicyBase):
     ...
     def test_09_MacDest_Policy_Forward_Rule_with_PVID_Override_0_Untagged_Packet(self):
         '''[Documentation]  Test_Objective: Verify_macdest_policy_drop_rule_discards_untagged_packets.'''
         ...
         self.localPolicyUdks.policy_set_rule_macdest(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_mac_a,forward=True)
         self.localPolicyUdks.policy_verify_rule_macdest_exists(self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.dst_mac_a,forward=True)
         ...
         self.localPolicyUdks.Transmit_100_Untagged_Frames_and_Verify_NOT_Received(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_a,self.tb.config.dst_mac_b,self.tb.config.vlan_a)
