...
class SysLogActionTests(PolicyBase):
    @mark.F_1000_0301
    @mark.EXOS
    @mark.P3
    @mark.Precedence_ACL
    ...
    @mark.NIGHTLY
    @mark.NO30_6   
    def test_01_Syslog_Action_for_Policy_Access_Lists(self):
        '''[Documentation]  Test_Objective: Verify_An_Access-List_sysprint('_actions_works_as_expected.'''
        
    
        self.localPolicyUdks.Policy_Common_Test_Case_Setup()
        print('  (Step_0) dump_syslog, clear_logs.')
        ...
        self.localPolicyUdks.vlanUdks.Add_Ports_to_Untagged_Egress_for_VLAN_and_Verify_it_is_Added(self.tb.config.netelem1.name, self.tb.config.vlan_b,
                                                                          self.tb.config.netelem1.tgen.port_c.ifname)
    
        
        print('  (Step_2) Create_A_conduit_for_the_Syslog_message_to_egress')
    #
    #   Put_an_IP_address_on_the_default_Vlan.
    #   Assign_a_FDB_entry_to_a_port_and_then_an_ARP_entry_to_the_FDB_entry
    #   Configure_and_enable_the_sysprint('_messages_to_be_setn_to_the_IP_address_in_the_ARP_entry.
    #
        self.localPolicyUdks.portUdks.Enable_Port_and_Validate_Port_is_Enabled( self.tb.config.netelem1.name,
                                                                                                self.tb.config.netelem1.tgen.port_c.ifname)
        self.localPolicyUdks.interfaceUdks.Configure_Primary_IPv4_Address_Enable_Interface_and_Validate(self.tb.config.netelem1.name,
               self.tb.config.vlan_b,   self.tb.config.vlan_default_ip,  '255.255.255.0')            
            
        self.localPolicyUdks.fdbUdks.Add_Static_FDB_Entry_and_Validate_It_Exists(self.tb.config.netelem1.name, self.tb.config.dst_mac_c, self.tb.config.vlan_b,
                                                                                     self.tb.config.netelem1.tgen.port_c.ifname)
            
                        
        self.localPolicyUdks.arpUdks.Add_ARP_Entry_and_Verify_it_was_Added(self.tb.config.netelem1.name,self.tb.config.dst_ip_c,
                                                                           self.tb.config.dst_mac_c)
        
        self.localPolicyUdks.networkElementCliSend.send_cmd(self.tb.config.netelem1.name, 'configure syslog add ' + self.tb.config.dst_ip_c + ' vr vr-Default local0')
        self.localPolicyUdks.networkElementCliSend.send_cmd(self.tb.config.netelem1.name, 'enable syslog')
        ...
        self.localPolicyUdks.networkElementPolicyGenKeywords.policy_set_access_list(self.tb.config.netelem1.name,'AclWithLog','ether',
                                                                                     'ether 0x0800','forward syslog')
            
        self.localPolicyUdks.networkElementPolicyGenKeywords.policy_verify_acl_ether(self.tb.config.netelem1.name,'AclWithLog','ether','0x800',
                                                                                     '16')
        
        self.localPolicyUdks.networkElementPolicyGenKeywords.policy_verify_acl_action_all(self.tb.config.netelem1.name,'AclWithLog','ether','NV','S','fwrd',
                                                                                          '', '')
        print('  (Step_6) Assign_ethertype (0x0800) Forward_access_list_rule_to_the_policy_profile. No_Cos_override (cos_3)')
        
        self.localPolicyUdks.change_policy_access_list_profile_index(self.tb.config.netelem1.name,self.tb.config.profile_a, 'AclWithLog')
    #
        ...
        self.localPolicyUdks.Transmit_100_Untagged_TCP_Frames_and_Verify_Received(
              ...
              self.tb.config.vlan_b,self.tb.config.src_ip_a, self.tb.config.dst_ip_a,self.tb.config.l4_port_a,self.tb.config.l4_port_c)
    
    #BGC_BUG_should_also_check_port_C_for_receipt_of_sysprint('_message_when_supported_by_Robot.
        time.sleep(int(self.tb.config.policy_delay))
          # Delay_to_allow_sysprint('_message_to_be_print('ged.       
        self.localPolicyUdks.networkElementLoggingGenKeywords.logging_verify_string_in_log(self.tb.config.netelem1.name,  'LogRuleHit')
    ...
    #         Verify_String_In_print('ging  self.tb.config.netelem1.name,  AclWithprint('_expect_error=true
    
    #        self.localPolicyUdks.networkElementCliSend.send_cmd(  self.tb.config.netelem1.name,  show_print(' | exclude_ConvEndPoint
    
        #self.Test_Case_Cleanup()#
    
    def Test_Case_Cleanup(self):
    #    Remove_All_Policy_Rules                          self.tb.config.netelem1.name,
        self.localPolicyUdks.Policy_Common_Test_Case_Cleanup() 
        ...
        #self.localPolicyUdks.networkElementCliSend.send_cmd(self.tb.config.netelem1.name,'del '+ self.tb.config.dst_ip_c  + ' vr vr-Default local0')
        self.localPolicyUdks.networkElementCliSend.send_cmd(self.tb.config.netelem1.name,'configure syslog delete all')
        #self.localPolicyUdks.networkElementCliSend.send_cmd(self.tb.config.netelem1.name,'disable syslog')
         
        self.localPolicyUdks.fdbUdks.Remove_Static_FDB_Entry_and_Validate_It_Does_Not_Exist(
                                    self.tb.config.netelem1.name,  self.tb.config.dst_mac_c,self.tb.config.vlan_b,self.tb.config.netelem1.tgen.port_c.ifname)
        ...
        #self.localPolicyUdks.interfaceUdks.Remove_Primary_IPv4_Address_Interface_and_Validate_Interface_Removed(self.tb.config.netelem1.name,  
                                                                                                                #self.tb.config.vlan_b)     
    #
        ...
        '''self.localPolicyUdks.vlanUdks.Remove_Ports_from_Untagged_Egress_for_VLAN_and_Verify_it_is_Removed(self.tb.config.netelem1.name,self.tb.config.vlan_b,
                                                                           self.tb.config.netelem1.tgen.port_c.ifname)'''
