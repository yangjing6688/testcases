...
class AuthWithoutActionSetCOATest(PolicyBase): 
    @mark.F_1000_0402
    @mark.EXOS
    @mark.P3
    @mark.Precedence_ACL
    ...
    def test_01_Access_List_COA_Single_Match_Full_Mask_Dropping_Tests(self):
        ...
        self.loacalPolicyUdks.Basic_Policy_Setup()
    ...
    #
        ...
        self.localPolicyUdks.networkElementCliSend.send_cmd( self.tb.config.netelem1.name,  'configure_policy_slices_tci-overwrite 1')
        self.localPolicyUdks.networkElementCliSend.send_cmd( self.tb.config.netelem1.name,  'configure_policy_slices_shared 1')
        self.localPolicyUdks.change_policy_rule_model(self.tb.config.netelem1.name, 'Access-List')
        self.localPolicyUdks.networkElementCliSend.send_cmd( self.tb.config.netelem1.name,  "disable_clipaging")
        ...
        self.localPolicyUdks.radiusSuiteUdks.Configure_And_Enable_Radius_Netlogin_Server(
          self.tb.config.netelem1.name,  self.tb.config.endsysRadius.instance,  self.tb.config.endsysRadius.ip,  self.tb.config.endsysRadius.ip,
    ...
    #
        ...
        self.localPolicyUdks.endsystemConnectionManager.connect_to_endsystem_element(self.tb.config.endsysRadius.name,  
                    self.tb.config.endsysRadius.ip, self.tb.config.endsysradius.username,  self.tb.config.endsysradius.password,
           self.tb.config.endsysradius.connection_method,  self.tb.config.endsysradius.os)
    ...
    #
        ...
        self.localPolicyUdks.radiusSuiteUdks.Send_COA_Auth_Request(
             ...
             self.tb.config.macAuthClient,'"v:1_t:a_m:ipproto=udp_a:drop"')
        ...
        self.localPolicyUdks.radiusSuiteUdks.Send_COA_Auth_Request(
             self.tb.config.endsysRadius.name,  self.tb.config.netelem1.ip,  self.tb.config.packetA.hyphenated_mac,  self.tb.config.policyName_a,  self.tb.config.vlan_400,  coaIfNum,  
             self.tb.config.macAuthClient,'"v:1_t:d_m:ipproto=udp_a:drop"')
    #
    #
    ...
    #
        ...
        self.localPolicyUdks.radiusSuiteUdks.Send_COA_Auth_Request(
             ...
             '"v:1_t:a_m:ipv4dst=self.tb.config.dst_ip_a,/32,l4dstport=self.tb.config.l4_port_c/16,ipproto=tcp_a:drop"')
        time.sleep(int(self.tb.config.policy_delay))
    
        print('  (Verification_8a) Verify_100_TCP_frames_with_dst_ip_a:l4_port_c_were_sent_and_NOT_received')
        self.localPolicyUdks.Transmit_100__Untagged_TCP_Frames_and_Verify_NOT_Received(
             self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,    self.tb.config.packetA.src_mac,    self.tb.config.packetA.dst_mac,      self.tb.config.vlan_100,
        ...
        self.localPolicyUdks.Transmit_100__Untagged_TCP_Frames_and_Verify_Received(
             self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,    self.tb.config.packetA.src_mac,    self.tb.config.packetA.dst_mac,     self.tb.config.vlan_100,
             self.tb.config.src_ip_a,    self.tb.config.dst_ip_a,    self.tb.config.l4_port_a,    self.tb.config.l4_port_b)
    
        print('  (Verification_8c) Verify_100_UDP_frames_with_dst_ip_a:l4_port_c_were_sent_and_received')
        self.localPolicyUdks.Transmit_100__Untagged_UDP_Frames_and_Verify_Received(
             self.tb.config.tgen_ports.netelem1.port_a,  self.tb.config.tgen_ports.netelem1.port_b,    self.tb.config.packetA.src_mac,    self.tb.config.packetA.dst_mac,     self.tb.config.vlan_100,
             self.tb.config.src_ip_a,    self.tb.config.dst_ip_a,    self.tb.config.l4_port_a,    self.tb.config.l4_port_c)
    
        self.localPolicyUdks.radiusSuiteUdks.Send_COA_Auth_Request(
             self.tb.config.endsysRadius.name,  self.tb.config.netelem1.ip,  self.tb.config.packetA.hyphenated_mac,  self.tb.config.policyName_a,  self.tb.config.vlan_400,  coaIfNum,  self.tb.config.macAuthClient,
             '"v:1_t:d_m:ipv4dst=self.tb.config.dst_ip_a,/32,l4dstport=self.tb.config.l4_port_c/16,ipproto=tcp_a:drop"')
    #
    #
    #
        ...
        self.localPolicyUdks.radiusSuiteUdks.Send_COA_Auth_Request(
             self.tb.config.endsysRadius.name,  self.tb.config.netelem1.ip,  self.tb.config.packetA.hyphenated_mac,  self.tb.config.policyName_a,  self.tb.config.vlan_400,  coaIfNum,  self.tb.config.macAuthClient,
             '"v:1_t:a_m:ipv4src=self.tb.config.src_ip_a,/32,l4srcport=self.tb.config.l4_port_a,/16,ipproto=tcp_a:drop"')
        time.sleep(int(self.tb.config.policy_delay))
        ...
        self.localPolicyUdks.radiusSuiteUdks.Send_COA_Auth_Request(
             self.tb.config.endsysRadius.name,  self.tb.config.netelem1.ip, self.tb.config.packetA.hyphenated_mac,  self.tb.config.policyName_a,  self.tb.config.vlan_400,  coaIfNum,  self.tb.config.macAuthClient,
             '"v:1_t:d_m:ipv4src=self.tb.config.src_ip_a,/32,l4srcport=self.tb.config.l4_port_a,/16,ipproto=tcp_a:drop"')
    #
    #
    #
    #
        ...
        self.localPolicyUdks.radiusSuiteUdks.Send_COA_Auth_Request(
             self.tb.config.endsysRadius.name,  self.tb.config.netelem1.ip,  self.tb.config.packetA.hyphenated_mac,  self.tb.config.policyName_a,  self.tb.config.vlan_400,  coaIfNum,  self.tb.config.macAuthClient,
             '"v:1_t:a_m:ipv4src=self.tb.config.src_ip_a,/32,l4srcport=self.tb.config.l4_port_a,/16,ipv4dst=self.tb.config.dst_ip_a,/32,l4dstport=self.tb.config.l4_port_b,/16,ipproto=tcp_a:drop"')
        time.sleep(int(self.tb.config.policy_delay))
        ...
        self.localPolicyUdks.radiusSuiteUdks.Send_COA_Auth_Request(
             self.tb.config.endsysRadius.name,  self.tb.config.netelem1.ip,  self.tb.config.packetA.hyphenated_mac,  self.tb.config.policyName_a,  self.tb.config.vlan_400,  coaIfNum,  self.tb.config.macAuthClient,
             '"v:1_t:d_m:ipv4src=self.tb.config.src_ip_a,/32,l4srcport=self.tb.config.l4_port_a,/16,ipv4dst=self.tb.config.dst_ip_a,/32,l4dstport=self.tb.config.l4_port_b,/16,ipproto=tcp_a:drop"')
    ...
    #    Remove_the_RADIUS_configuration
        ...
        self.localPolicyUdks.networkElementCliSend.send_cmd( self.tb.config.netelem1.name,  'configure_netlogin_del_mac-list_ff:ff:ff:ff:ff:ff_48')
        ...
        self.localPolicyUdks.policyUdks.Disable_Policy_and_Verify_it_is_Disabled(  self.tb.config.netelem1.name)
        self.localPolicyUdks.networkElementCliSend.send_cmd( self.tb.config.netelem1.name,  'configure_policy_slices_shared_0')
        self.localPolicyUdks.networkElementCliSend.send_cmd( self.tb.config.netelem1.name,  'configure_policy_slices_tci-overwrite_4')
