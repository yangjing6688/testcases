...
class CosStatusTciOverwriteEnabledTests(PolicyBase):
     @mark.F_1000043383
     @mark.EXOS
     @mark.P2
     @mark.Policy_Profile
     def test_07_Policy_Profile_CoS_Status_and_TCI_Overwrite_Enabled(self):
         '''[Documentation]  Test_Objective: Verify_when_TCI_overwrite_is_enabled_and_cos-status_is_enabled_the_tagged packet_is_associated_to_the_policy_cos'''
         ...
         self.localPolicyUdks.Create_Policy_Profile_with_COS(COS_Status,and_TCI_Overwrite_Enabled,self.tb.config.netelem1.name,self.tb.config.profile_a,self.tb.config.cos_5)
         ...
         self.localPolicyUdks.Transmit_100_Tagged_Frames_and_Verify_Priority_Changed(self.tb.config.tgen_ports.netelem1.port_a,self.tb.config.tgen_ports.netelem1.port_b,self.tb.config.src_mac_b,self.tb.config.dst_mac_a,self.tb.config.vlan_a,self.tb.config.priority_2,self.tb.config.priority_5)
