...
class EgressVlansTests(PolicyBase):
     @mark.F_1000043386
     @mark.EXOS
     @mark.P2
     @mark.Policy_Profile
     def test_10_Policy_Profile_Egress_VLANs(self):
         '''[Documentation]  Test_Objective: Verify_a_policy_profile_configured_with_egress-vlans_that_traffic_is_egressed_to that_vlan.'''
         ...
         print('  (Verification_1) Verify_100_untagged_frames_were_sent_and_received_with_a_VID_of_4052.')
         ...
         print('  (Verification_2) Verify_100_untagged_frames_were_sent_and_received_with_a_VID_of_4052.')
         ...
         print('  (Verification_3) Verify_Policy_added_the_ports_to_the_tagged_egress_list_for_vlan_b.')
         self.localPolicyUdks.Port_Should_be_on_Tagged_Egress(self.tb.config.netelem1.name,self.tb.config.vlan_b,self.tb.config.netelem1.tgen.port_b.ifname)
