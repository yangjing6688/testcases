...
class RadiusSuiteUdks(RadiusUdks):
    ...
    def UnConfigure_And_Disable_Radius_Netlogin_Server(device_name, Instance):
        self.Remove_Radius_Server_and_Verify_it_is_Removed(NetElem, Instance)
    ...
    def Send_COA_Auth_Request(self, device_name, EndSysName, Ip, HyphenatedMac,PolicyProfile = '-', RFC3580Vlan = '4096', NasPort = '0',
     ...
     # [Arguments]  ${EndSysName}  ${Ip}  ${HyphenatedMac}
      ...
      cmd_str = '/root/RobotScripts/COA-Auth-Send ' + str(Ip) + ' ' + str(HyphenatedMac) + ' ' + str(no_policy_profile) + ' ' + str(no_vlan) + ' '+ str(no_nas_port) + ' ' + str(no_auth_client) +' ' + str(no_policy_acl)
      #self.networkElementCliSend.send_cmd(device_name, '/root/RobotScripts/COA-Auth-Send' + str(Ip) ${HyphenatedMac} ${PolicyProfile} ${RFC3580Vlan} ${NasPort} ${AuthClient} ${PolicyAcl} )
      
      try :
          self.networkElementCliSend.send_cmd(device_name,cmd_str)
      except:
          pass
