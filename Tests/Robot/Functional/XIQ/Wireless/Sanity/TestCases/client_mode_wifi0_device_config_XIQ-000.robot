...
# Description   : TCXM-15115
#                 Enable Client mode for WIFI0 under Device Configuration and Add "Client Mode Profile"
#########################################################################################################
...
Step1: Create Policy and Update Policy to AP1 and AP2
    ...
    [Tags]              tcxm-15115   development   step1   steps
...
Step2: Client mode enable in device Configuration for AP2
    [Documentation]     Client mode enable in device Configuration for AP2
    [Tags]              tcxm-15115   development   step2   steps
...
Step3: Setup WIFI on STA2 and Connect to AP2
    [Documentation]     Setup WIFI on STA2 and Connect to AP2 on Client Mode
    [Tags]              tcxm-15115   development   step3   steps

    Depends On Test     Step2: Client mode enable in device Configuration for AP2
    Setup AP in Client Mode          ${ap2}
    mu1.connect wpa2 ppsk network    ${SSID_CM}          aerohive

Step4: Verify Client mode and a Client Connection
    [Documentation]     Verify Client mode and a Client Connection
    [Tags]              tcxm-15115   development   step4   steps

    Depends On Test     Step3: Setup WIFI on STA2 and Connect to AP2
    sleep               20s
