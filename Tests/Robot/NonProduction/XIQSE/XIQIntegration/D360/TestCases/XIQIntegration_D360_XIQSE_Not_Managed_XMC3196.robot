...
#                 when the Site Engine is not modeled as an SNMP-managed device.
#                 This is qTest test case (CSIT project):
#                   TC-9037: D360 View:XIQ Site Engine - no managed device
...
Test 1: Confirm Basic Device360 Values
    ...
    [Tags]              nightly2    release_testing    tccs_9037   xmc_3196    development    known_issue    xiqse    xiq_integration    d360    not_managed    test1
    ...
    Confirm Device360 Ports Information Is Not Displayed
...
Test 2: Confirm Alarm and Event Device360 Values
    ...
    [Tags]              nightly2    release_testing    tccs_9037   xmc_3196    development    known_issue    xiqse    xiq_integration    d360    not_managed    test2
...
Remove Existing Site Engine from XIQ
    ...
    ${search_result}=  Search Device   device_mac=${XIQSE_MAC}     ignore_failure=True
...
Confirm Device360 Title for XIQ Site Engine
    ...
    Should Be Equal  ${title}  ${XIQSE_PRODUCT} - ${XIQSE_IP}
...
Confirm Device360 Top Bar Information for XIQ Site Engine
    ...
    ${topbar_mem}=      Get From Dictionary  ${topbar_info}  memory_usage
    ${topbar_mac_use}=  Get From Dictionary  ${topbar_info}  mac_usage
    ${topbar_temp}=     Get From Dictionary  ${topbar_info}  temperature
    ...
    Should Be Equal                 ${topbar_mem}       0%
    Should Be Equal                 ${topbar_mac_use}   0%
    Should Be Equal                 ${topbar_temp}      0
...
#    Wait Until Keyword Succeeds  15x  10 sec  Compare Last Update Times
    Log To Console  Need to add in check for Last Update Times - XMC-3467
...
Compare Last Update Times
    ...
    Switch To Window  ${XIQSE_WINDOW_INDEX}
    Navigate to XIQ Device Message Details and Confirm Success
    ${uptime_val}=  XIQSE XIQ Device Message Details Get Last Update Time    ${XIQSE_IP}  ${XIQSE_PRODUCT}
    ...
    ${uptime_compare}=  Compare Date Time Strings  ${topbar_uptime}  ${uptime_val}
    Should Be Equal As Integers     ${uptime_compare}   1
...
Confirm Device360 Ports Information Is Not Displayed
    [Documentation]     Confirms the ports table area of the Device360 view is empty
    ...
    Should Be Equal As Integers     ${table_displayed}    0
